...
MainLoop(gpointer data)
{
  ...
  */
  ...
  /* Create some lists */
  {
    ...
    if (pmodified)
    {
      ...
      */
      ...
      for (i = 0; i < GameParametres(GET, GNPLAYERS, 0) + 2; i++)
      {
        if (ps[i].status >= PLAYERACTIVE)
          ps[i].status = PLAYERACTIVE;
      }
    }
    lasttimeupdate++;
  }
  ...
  if (GameParametres(GET, GPIRATES, 0) == TRUE)
  {
    ...
    {
      if (proc == ps[GameParametres(GET, GNPLAYERS, 0) + 1].proc)
      { /*  Send TO ai */
        if (lasttimepirates > timenow)
        {
          lasttimepirates = timenow;
        }
        if (timenow - lasttimepirates > 2000)
        { /* 2000 */
...
#if TEST
            snprintf(text, MAXTEXTLEN, "%s: %d %d %f", GetLocale(L_PIRATESATSECTOR), (int)(x0 / SECTORSIZE), (int)(y0 / SECTORSIZE), level);
#else
            snprintf(text, MAXTEXTLEN, "%s: %d %d", GetLocale(L_PIRATESATSECTOR), (int)(x0 / SECTORSIZE), (int)(y0 / SECTORSIZE));
#endif
    ...
    if (timenow - lasttimeasteroids > 3000)
    {
      ...
      { /* every 10 minutes */
        ...
        for (i = 0; i < factor; i++)
        {
          ...
          if (Distance2NearestShip(&listheadobjs, -1, x0, y0) < MAXASTEROIDDISTANCE2)
          { /* 20 sectors */
            ...
            for (j = 1; j < GameParametres(GET, GNPLAYERS, 0) + 2; j++)
            {
              ...
              {
                snprintf(text, MAXTEXTLEN, "%s: %d %d", GetLocale(L_ASTEROIDSATSECTOR), (int)(x0 / SECTORSIZE), (int)(y0 / SECTORSIZE));
...
gint
Quit(GtkWidget* widget, gpointer gdata)
{
  /*
  ...
  if (1)
  {
    ...
    printf("****************** Statistics ************************\n");
    obj = NewObj(SHIP, EXPLORER, 0, 0, 0, 0, CANNON0, ENGINE0, 0, NULL, NULL);
    Add2ObjList(&listheadobjs, obj);
    if (obj != NULL)
    {
      if (1)
        printf("Last obj id: %d\n", obj->id);
      obj = NewObj(PROJECTILE, EXPLOSION, 0, 0, 0, 0, CANNON0, ENGINE0, obj->player, obj, NULL);
      Add2ObjList(&listheadobjs, obj);
      if (obj != NULL)
      {
  ...
  switch (navcontrol)
  {
    ...
    case TRUE: /* Nav mode */
      ...
      if (ship_c != NULL)
      {
        ...
        if (ship_c->ai == 0 && proc == ps[ship_c->player].proc)
        {
          ...
          if (key->s == TRUE && key->ctrl == FALSE)
          {
            if (0)
            {
              nobj = NewObj(SHIP, SATELLITE, ship_c->x - 2.5 * ship_c->radio * Cos(ship_c->a), ship_c->y - 2.5 * ship_c->radio * Sin(ship_c->a), ship_c->vx, ship_c->vy, CANNON4, ENGINE1, ship_c->player, ship_c, ship_c->in);
              if (nobj != NULL)
              {
                nobj->ai = 1;
      ...
      if (cv != NULL)
      {
        ...
        if (TEST || actual_player == actual_player0)
        { /* PRODUCTION */
...
UpdateShip(Object* obj)
{
  ...
  */
  ...
  if (proc == ps[obj->player].proc)
  {
    ...
    if (obj->type == SHIP && obj->mode == LANDED && obj->subtype != PILOT)
    { /* HERE TODO add propertie to ships, pilots. */
      ...
      /***** Learning, experience *****/
      if (obj->type == SHIP && obj->state > 99 && obj->subtype != PILOT)
      { /* HERE TODO pilots has no ship for training. must depend of pilot properties. */
        int mlevel = 0;
        float incexp;
                    ...
                    if (IsInIntList((ps[obj->player].kplanets), pnt->id) == 0)
                    {
                      ...
                      for (i = 0; i <= gnplayers + 1; i++)
                      {
                            ...
                            (genemyknown && ps[obj->player].team > 1))
                        {
                          ...
                          if (ps[obj->player].team == ps[i].team)
                          {
                            snprintf(text, MAXTEXTLEN, "(%d) %s %d %s", obj->pid, GetLocale(L_PLANET), pnt->id, GetLocale(L_DISCOVERED));
            ...
            if ((obj1->type != PLANET && obj2->type != PLANET))
            {
              ...
              ******/
              ...
              for (i = 0; i < 2; i++)
              {
                ...
                if (objt1->type == SHIP || objt1->type == ASTEROID || (objt1->type == PROJECTILE && objt1->subtype == MISSILE))
                { /* HERE */
                  if (gnet == TRUE)
                  {
                    if (proc == ps[objt1->player].proc)
                    ...
                    case ASTEROID:
                      if (objt1->subtype < ASTEROID3)
                      {
                        if (proc == ps[objt1->player].proc)
                        {
                          for (j = 0; j < 3; j++)
                          {
                            nobj = NewObj(ASTEROID, objt1->subtype + 1, objt1->x, objt1->y, objt1->vx + 10.0 * rand() / RAND_MAX - 5, objt1->vy + 10.0 * rand() / RAND_MAX - 5, CANNON0, ENGINE0, 0, NULL, objt1->in);
                            if (nobj != NULL)
                            {
                              nobj->a = PI / 2;
                              nobj->ai = 0;
                  ...
                  case LANDZONE:
                    ...
                    { /***** ship has landed *****/
                      if (obj1->vy < 0)
                      {
                        ...
                        { /* if has a rescued pilot, eject him */
                      ...
                      switch (IsPlanetEmpty(obj1->in, obj1))
                      {
                        case 0: /* planet totally empty */
                          if (ps[obj1->in->player].team != ps[obj1->player].team)
                          {
                            ...
                            if (ps[obj1->in->player].team != 1)
                            { /* HERE ??? */
                              GetInformation(&ps[obj1->player], &ps[obj1->in->player], obj1);
                            }
                            snprintf(text, MAXTEXTLEN, "%s %d %s", GetLocale(L_PLANET), obj1->in->id, GetLocale(L_LOST));
          ...
          if (1)
          {
            ...
            if (obj->parent != NULL)
            {
              ...
              { /* 16 */
                ...
                nobj = NewObj(PROJECTILE, SHOT1, obj->x, obj->y, vx, vy, CANNON0, ENGINE0, obj->player, obj->parent, obj->in);
                if (nobj != NULL)
                {
                  nobj->durable = TRUE;
        ...
        case SHIP:
          if (obj->trace == TRUE && obj->mode != LANDED)
          {
            if (obj->vx * obj->vx + obj->vy * obj->vy > .25)
            {
              ...
              if (objin != NULL)
              { /* don't create trace inside a ship */
                if (obj->in->type != PLANET)
                {
                  objin = NULL;
                }
              }
              nobj = NewObj(TRACE, SHIP0, obj->x, obj->y, 0, 0, CANNON0, ENGINE0, obj->player, obj, objin);
              if (nobj != NULL)
              {
                Add2ObjList(&listheadobjs, nobj);
              }
...
int
CheckPlanetDistance(struct HeadObjList* lh, float x, float y, int d)
{
  /*
...
void
CreateUniverse(int ulx, int uly, struct HeadObjList* lheadobjs, char** ptnames)
{
  /*
...
int
PlanetAtraction(float* fx, float* fy, float x, float y, float m)
{
  /*
...
int
PrintfObjInfo(FILE* fp, Object* obj)
{
  /*
    printf obj info
    used only for debug.
...
Object*
ChooseInitPlanet(struct HeadObjList lheadobjs)
{
  /*
    returns:
  ...
  */
  ...
  do
  {
    ...
    while (ls != NULL)
    {
      if (ls->obj->type == PLANET)
      {
        if (ls->obj->player == 0 && cont == rplanet)
        { /* a random planet */
          return (ls->obj);
        }
      }
...
void
CreateShips(struct HeadObjList* lheadobjs)
{
  /*
...
void
CreateTestShips(struct HeadObjList* lheadobjs)
{
  /*
  ...
  */
  ...
  { /* asteroids */
    for (i = 0; i < 4; i++)
    {
      ...
      x = 450 * Sin(a);
      y = 450 * Cos(a);
      obj = NewObj(ASTEROID, ASTEROID1, 0, 0, 0, 0, CANNON0, ENGINE0, 2, NULL, NULL);
      if (obj != NULL)
      {
        obj->x = obj->x0 = x;
        obj->y = obj->y0 = y;
  ...
  { /* five static ships */
    for (i = 0; i < 5; i++)
    {
      ...
      x = 400 * Sin(a);
      y = 400 * Cos(a);
      obj = NewObj(SHIP, FIGHTER, 0, 0, 0, 0, CANNON4, ENGINE3, 2, NULL, NULL);
      if (obj != NULL)
      {
        obj->x = obj->x0 = x;
        obj->y = obj->y0 = y;
  ...
  x = 0;
  y = 0;
  obj = NewObj(SHIP, FIGHTER, 0, 0, 0, 0, CANNON4, ENGINE3, 1, NULL, NULL);
  if (obj != NULL)
  {
    obj->x = obj->x0 = x;
    obj->y = obj->y0 = y;
...
void
GetPoints(struct HeadObjList* hol, int proc, struct Player* p)
{
  /*
...
void
GetPointsObj(struct HeadObjList* lhobjs, struct Player* p, Object* obj)
{
  /*
     version 0.3    12Dic2010
     obj   dead object
...
void
SetGameParametres(struct Parametres param)
{
  int width, height;
...
void
MakeTitle(struct Parametres param, char* title)
{
...
void
AddPlanets2List(struct HeadObjList* listheadobjs, struct Player* players)
{
  /*
