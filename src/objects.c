...
Object*
NewObj(int type, int stype, int x, int y, float vx, float vy, int weapontype, int engtype, int player, Object* parent, Object* in)
{
  ...
  */
  ...
  /* data base */
  if (obj->type == SHIP)
  { /* HERE TODO SHIP COMPUTER, only ships */
...
int
Add2ObjList(struct HeadObjList* lhead, Object* obj)
{
  /*
...
int
Add2ObjList_B(struct HeadObjList* lhead, Object* obj)
{
  /*
...
int
Add2ObjList_E(struct HeadObjList* lhead, Object* obj)
{
  /*
...
void
NewWeapon(Weapon* weapon, int type)
{
  /*
...
int
CountObjs(struct HeadObjList* lh, int player, int type, int subtype)
{
  /*
...
int
CountModObjs(struct HeadObjList* lh, int type)
{
  /*
...
Object*
RemoveDeadObjs(struct HeadObjList* lhobjs, Object* cv0)
{
  /*
     version 0.3
  ...
  */
  ...
  while (ls != NULL)
  {
    ...
    if (ls->obj->type == PROJECTILE && ls->obj->subtype == MISSILE && ls->obj->state <= 0)
    {
      swdead = 1;
    }
    ...
    if (swdead)
    {
      ...
      switch (ls->obj->type)
      {
        case ASTEROID:
          ...
          if (ls->obj->habitat == H_PLANET && ls->obj->sw == 0)
          { /* asteroid crashed */
        ...
        case SHIP:
          ...
          if (ls->obj->habitat == H_PLANET && swx)
          {
            ...
            { /* ship crashed */
              ...
              if (factor < .2)
                factor = .2; /* at least an 20 percent*/
...
void
RemoveObj(struct HeadObjList* lhobjs, Object* obj2remove)
{
  /*
...
int
CountPlayerShipObjs(struct HeadObjList* lh, int player, int* cont)
{
  /*
...
int
GetLandedZone(Segment* segment, struct Planet* planet)
{
...
int
GetSegment(Segment* segment, struct Planet* planet, float x, float y)
{
  /*
...
void
Explosion(struct HeadObjList* lh, Object* cv, Object* obj, int type)
{
  /*
  ...
  */
  ...
  switch (type)
  {
    case 0: /* ship destroyed */
      ...
      n += nexplosion;
      for (i = 0; i < nexplosion; i++)
      { /* 16 */
        a = 2. * PI * (Random(-1));
        v = 1.0 * VELMAX * (Random(-1));
        vx = v * Cos(a) + obj->vx;
        vy = v * Sin(a) + obj->vy;
        nobj = NewObj(PROJECTILE, EXPLOSION, obj->x, obj->y, vx, vy, CANNON0, ENGINE0, obj->player, obj, obj->in);
        if (nobj != NULL)
        {
...
int
CountPlayerPlanets(struct HeadObjList* lh, struct Player* player, int* cont)
{
  /*
    version 0.1
...
int
CountPlanets(struct HeadObjList* lh, int type)
{
  /*
...
int
CountShipsInPlanet(struct HeadObjList* lh, int objid, int player, int type, int stype, int max)
{
  /*
...
int
CountShips(struct HeadObjList* lh, int* planet, int* ships)
{
  /*
...
int
CopyObject(Object* nobj, Object* obj)
{
...
Object*
SelectObj(struct HeadObjList* lh, int id)
{
  /*
     returns:
...
Object*
SelectpObj(struct HeadObjList* lh, int pid, int player)
{
  /*
     returns:
     a pointer to the Object with the id id,
...
Object*
SelectObjInObj(struct HeadObjList* lh, int id, int player)
{
  /*
     returns:
...
Object*
SelectpObjInObj(struct HeadObjList* lh, int pid, int player)
{
  /*
     returns:
     a pointer to the first Object which is inside of id
...
Object*
SelectOneShip(struct HeadObjList* lh, Space reg, Object* cv, int ctrl)
{
  /*
  ...
  */
  ...
  while (ls != NULL)
  {
    ...
    if (reg.habitat > 0)
    { /* select a region inside a planet */
...
int
IsInRegion(Object* obj, Space region)
{
  Point a, b;
  Rectangle rect;
...
float
Distance2NearestShip(struct HeadObjList* lh, int player, int x, int y)
{
  /*
...
float
Distance2NearestShipLessThan(struct HeadObjList* lh, int player, int x, int y, float dmin2)
{
  /*
...
Object*
ObjNearThan(struct HeadObjList* lh, int player, int x, int y, float d2)
{
  /*
    returns:
...
Object*
NearestObj(struct HeadObjList* lh, Object* obj, int type, int stype, int pstate, float* d2)
{
  /*
  ...
  *d2 = -1;
  ...
  while (ls != NULL)
  {
    ...
    if (pstate & PENEMY)
    {
      if ((players[obj2->player].team != players[player].team) && obj2->player != 0)
      {
        sw = 1;
      }
    }
...
void
NearestObjAll(struct HeadObjList* lhc, Object* obj, struct NearObject* objs)
{
  /*
    version 03
  ...
  for (j = 0; j < nlist; j++)
  {
    ...
    while (ls != NULL)
    {
        ...
        case ASTEROID:
          ...
          if (obj->habitat == H_PLANET && obj2->habitat == H_PLANET)
          {
            if (obj->in != obj2->in)
            {
              /* unupdated list */
#if TEST
              fprintf(stderr, "WARNING in NearestObjAll()unupdated list \n");
#endif
              ls = ls->next;
              continue;
            }
      ...
      switch (obj2->type)
      {
        ...
        case PLANET:
          if (obj2->player != 0 && (players[obj2->player].team != players[player].team))
          { /* Enemy Planet */
            if (objs[1].obj == NULL)
            {
              objs[1].obj = obj2;
...
void
DestroyAllObj(struct HeadObjList* lh)
{
...
void
DestroyAllPlayerObjs(struct HeadObjList* lh, int player)
{
  struct ObjList* ls;
...
void
DestroyPlanet(struct Planet* planet)
{
  Segment *s, *s0;
...
int
CopyPlanet2Buf(struct Planet* planet, char* buf)
{
...
int
CopyBuf2Planet(char* buf, struct Planet* planet)
{
  struct Planet* planet2;
  Segment *s, *s2;
  int offset = 0;
...
int
UpdateSectors(struct HeadObjList lh)
{
  /*
...
int
Add2TextMessageList(struct TextMessageList* listhead, char* cad, int source, int dest, int mid, int time, int priority)
{
  /*
  ...
  */
  ...
  while (lh->next != NULL)
  {
    ...
    if (lh->next->info.source == source && lh->next->info.id == mid && lh->next->info.value == priority)
    { /* already added */
      return (1);
    }

    if (lh->next->info.value < priority)
    { /* added before end */
      /* printf("BREAK\n"); */
      break;
    }
...
int
GetPrice(Object* obj, int stype, int eng, int weapon)
{
  /*
    return:
...
int
BuyShip(struct Player* player, Object* obj, int stype)
{
  /*
  ...
  */
  ...
  if (obj->type == SHIP && obj->subtype == PILOT && stype != FIGHTER)
  { /* pilot buy a ship */
  ...
  if (obj->type == SHIP && stype == CLOAKDEVICE)
  { /* ship buys a cloaking device */
  ...
  {
    case EXPLORER:
      obj_b = NewObj(SHIP, EXPLORER, obj->x, s->y0, 0, 0, CANNON3, ENGINE3, obj->player, NULL, obj->in);
      break;
    case FIGHTER:
      obj_b = NewObj(SHIP, FIGHTER, obj->x, s->y0, 0, 0, CANNON4, ENGINE4, obj->player, NULL, obj->in);
      break;
    case TOWER:
      obj_b = NewObj(SHIP, TOWER, obj->x, s->y0, 0, 0, CANNON4, ENGINE1, obj->player, NULL, obj->in);

      break;
    case FREIGHTER:
      obj_b = NewObj(SHIP, FREIGHTER, obj->x, s->y0, 0, 0, CANNON0, ENGINE6, obj->player, NULL, obj->in);
    ...
    case SATELLITE:
      ...
      obj_b = NewObj(SHIP, SATELLITE, obj->x, s->y0, 0, 0, CANNON3, ENGINE1, obj->player, NULL, obj->in);

      break;
    case GOODS:
      obj_b = NULL;

      obj_b = NewObj(SHIP, GOODS, obj->x, s->y0, 0, 0, CANNON0, ENGINE0, obj->player, NULL, obj->in);
...
Object*
NextCv(struct HeadObjList* lh, Object* cv0, int pid)
{
  /*
    returns:
...
Object*
PrevCv(struct HeadObjList* lh, Object* cv0, int pid)
{
  /*
    returns:
    if cv0 is NULL a pointer to the first ship
...
Object*
FirstShip(struct HeadObjList* lh, Object* cv0, int pid)
{
  /*
    returns:
...
Object*
PrevPlanetCv(struct HeadObjList* lh, Object* cv0, int playerid)
{
  /*
    version 01
    returns:
...
Object*
NextPlanetCv(struct HeadObjList* lh, Object* cv0, int playerid)
{
  /*
    returns:
    if cv0 is NULL a pointer to the first ship
...
int
CountObjList(struct HeadObjList* hlist)
{
  /*
    returns:
...
int
DestroyObjList(struct HeadObjList* hl)
{
  /*
...
int
PrintObjList(struct HeadObjList* hl)
{
  /*
...
int
IsInObjList(struct HeadObjList* lhobjs, Object* obj)
{
...
void
KillAllObjs(struct HeadObjList* lheadobjs)
{
  /*
...
int
CreatePlayerList(struct HeadObjList hlist1, struct HeadObjList* hlist2, int player)
{
  /*
     version 04
...
int
CreateContainerLists(struct HeadObjList* lh, struct HeadObjList* hcontainer)
{
  /*
...
int
CreatekplanetsLists(struct HeadObjList* lh, struct HeadObjList* hkplanets)
{
  /*
...
int
CreatePlanetList(struct HeadObjList lheadobjs, struct HeadObjList* lheadplanets)
{
  /*
    version 0.1
...
void
CreateNearObjsList(struct HeadObjList* lh, struct HeadObjList* lhn, int player)
{
  /*
...
void
Experience(Object* obj, float pts)
{
  /*
...
void
FPrintObj(FILE* f, Object* obj)
{
  int n;
  n = CountOrders(obj);
  fprintf(f, "obj id:%d pid: %d type: %d stype: %d\n state: %f mode:%d player: %d norders: %d\n", obj->id, obj->pid, obj->type, obj->subtype, obj->state, obj->mode, obj->player, n);
}
...
Object*
MarkObjs(struct HeadObjList* lh, Space reg, Object* cv, int ctrl)
{
  /*
...
void
UnmarkObjs(struct HeadObjList* lh)
{
  /*
    unmark all objects
  */
...
int
PrintSelected(struct HeadObjList* lh)
{
  /*
...
int
NearMaxLevelObj(Object* obj, struct HeadObjList* lh)
{
  /*
     Not USED.
...
int
IsPlanetEmpty(Object* planet, Object* obj)
{
  /*
     don't count obj
     don't count pilots
  ...
  while (ls != NULL)
  {
    if (ls->obj->in == planet)
    {
      ...
      if (players[ls->obj->player].team != players[obj->player].team)
      {
        ...
        /* ls->obj->player,ls->obj->type,ls->obj->subtype,ls->obj->pid,
           ls->obj->mode,ls->obj->in->id,ls->obj->state,ls->obj->ttl); */
        return (2);
      }
      else
      {
...
int
UpdateCell(struct HeadObjList* lh, int* cell)
{
  /*
     version 02 080411
  */
...
int
ValueCell(int* cell, Object* obj, int gulx, int guly)
{
  /*
...
void
ShipProperties(Object* obj, int stype, Object* in)
{
  /*
...
int
CountSelected(struct HeadObjList* lh, int player)
{
  /*
    printf the selected ships
    return the number of selected ships
...
int
CountNSelected(struct HeadObjList* lh, int player)
{
  /*
...
Object*
FirstSelected(struct HeadObjList* lh, int player)
{
  /*
    printf the selected ships
...
float
Distance2(Object* obj1, Object* obj2)
{
  /*
    version 0.2
...
int
GameOver(struct HeadObjList* lhead, struct Player* players, int player)
{
  /*
  ...
  */
  ...
  {
    if (n == CountObjs(lhead, player, SHIP, PILOT))
...
int
CargoAdd(Object* obj1, Object* obj2)
{
  /*
  ...
  */
  ...
  {
    fprintf(stderr, "No room for %d in %d %d %d\n", obj2->id, obj1->id, obj2->mass + obj2->cargo.mass, obj1->cargo.capacity);
    return (0);
  }
...
Object*
CargoGet(Object* obj1, int type, int subtype)
{
...
int
CargoRem(Object* obj1, Object* obj2)
{
...
void
CargoCheck(struct HeadObjList* hol, Object* cvobj)
{
  /*
...
int
CargoEjectObjs(Object* obj, int type, int subtype)
{
  /*
...
int
CargoIsObj(Object* obj, Object* obj2)
{
  /*
  ...
  */
  ...
  if (obj->cargo.n <= 0)
  {
    fprintf(stderr, "ERROR in cargo list %d %d (%d,%d) %f\n", obj->player, obj->id, obj->type, obj->subtype, obj->state);
    fprintf(stderr, "\tERROR in cargo list %d %d (%d,%d) %f\n", obj2->player, obj2->id, obj2->type, obj2->subtype, obj2->state);
  }
  ls = obj->cargo.hlist->list;
...
CargoPrint(Object* obj)
{
  ...
  */
  ...
  printf("cargo: %d (%d,%d,%d)->", obj->id, obj->cargo.mass, obj->cargo.capacity, obj->cargo.n);
  while (ls != NULL)
  {
    n++;
...
struct ObjTree*
Add2ObjTree(struct ObjTree* head, Object* obj)
{
  /*
...
void
DestroyTree(struct ObjTree* head)
{
...
struct ObjTree*
DelObjTree(struct ObjTree* head, Object* obj)
{
  /*
...
void
PrintObjTree(struct ObjTree* head)
{
  /*
    version 01
...
int
IsInObjTree(struct ObjTree* head, int id)
{
  /*
...
struct ObjTree*
Look4ObjTree(struct ObjTree* head, Object* obj)
{
  /*
...
void
PrintVerletList(struct VerletList* hvl)
{
  /*
...
void
DestroyVerletList(struct VerletList* hvl)
{
  /*
