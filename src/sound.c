...
int
PlaySound(int sid, int mode, float vol)
{
  ...
  if (bufferid == -1)
  {
    ...
    if (data != NULL)
    {
      ...
      printf("PlaySound():sound added: %s to buffer: %d(%d) in data: %p\n",
             filesoundnames[buffertable[bufferfree]],
             buffers[bufferfree],
             bufferfree,
             (void*)data);
#endif
  ...
  if (sourceid == -1)
  { /* there are not a assigned source */
...
#if DEBUG
  if (debugsound)
    printf("Sound(): entering Sound() mode: %d sid:%d\n", mode, sid);
#endif
  switch (sid)
  {
    case MUSIC:
...
#if DEBUG
        if (debugsound)
          printf("Sound(): no accepting mode\n");
#endif
        return (0);
      }
...
float
SetMasterVolume(float vol, int action)
{
...
float
SetSoundVolume(float vol, int action)
{
  /* sets the effects volume */
  int i;
  ALfloat nvol = 0;
...
float
SetMusicVolume(float vol, int action)
{
...
ALvoid*
LoadMemoryFromOggFile(char* fname, ALenum* format, ALsizei* size, ALfloat* frequency)
{
  char* data;
