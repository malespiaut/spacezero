...
int
OpenComm(int mode, struct Parametres par, struct Sockfd* sockfd)
{
  /*
...
int
StartComm(int mode, struct Sockfd* sockfd)
{
  ...
  switch (mode)
  {
    ...
    case 1: /* client */
      ...
      fprintf(stdout, "Game:\n\tnplayers:%d \n\t npcc:%d\n\t npcs:%d\n", GameParametres(GET, GNPLAYERS, 0), npcc, npcs);
...
void*
CommServer(void /* struct Thread_arg */* a)
{
  /*
...
void*
CommClient(void /*struct Thread_arg*/* a)
{
  /*
    version 01 25Nov2010
    Client Thread
...
void
PrintTextMessage(struct Message* mess)
{
...
int
CopyObjs2Buffer(struct Buffer* buffer, struct HeadObjList hl)
{
  /*
    version 02 251110
  ...
  */
  ...
  while (ls != NULL)
  {
    ...
    {
      fprintf(stdout, "CopyObj2Buffer obj: %d not changed\n", obj->id);
    }
#endif
    ls = ls->next;
  }
...
int
CopyObj2Buffer(struct Buffer* buffer, void* object, int modtype)
{
  /*
    version 01 (011210)
    ...
    ******/
    ...
    switch (header.id)
    {
      ...
      case SENDOBJMOD0: /* only position */
        ...
        { /* New object or object has been killed in client side*/
      ...
      case SENDOBJNEW:
        ...
        nobj = NewObj(objnew.type, objnew.subtype, objnew.x, objnew.y, objnew.vx, objnew.vy, CANNON0, objnew.engtype, objnew.player, parent, NULL);
      ...
      case SENDMESS:
      {
        ...
        switch (mess.id)
        {
                ...
                if ((i != obj->player) && ((ps[obj->player].team == ps[i].team) || GameParametres(GET, GENEMYKNOWN, 0)))
                {
                  if (GetProc() == ps[i].proc)
                  {
                    if (IsInIntList((ps[i].kplanets), pnt->id) == 0)
                    {
                      ...
                      snprintf(text, MAXTEXTLEN, "(%s) %s %d %s", ps[obj->player].playername, GetLocale(L_PLANET), pnt->id, GetLocale(L_DISCOVERED));
          ...
          case NMPLANETLOST:
          {
            ...
            {
              snprintf(text, MAXTEXTLEN, "%s %d %s", GetLocale(L_PLANET), mess.b, GetLocale(L_LOST));
...
int
CopyObjOrdersfromBuffer(Object* obj0, char* buf0)
{
  ...
  for (i = 0; i < norders; i++)
  {
    ...
    fprintf(stdout, "%g %g %g %g\n%g %g %g %g\n", order.a, order.b, order.c, order.d, order.e, order.f, order.g, order.h);
#endif
  }
#if COMMDEBUG
  fprintf(stdout, "n orders readed:%d\n", nordread);
#endif
...
int
SetModified(Object* obj, int mode)
{
  /*
    version 01
    returns:
  ...
  if (GameParametres(GET, GMODE, 0) == LOCAL)
  {
    ...
    if (debugcomm1)
    {
      fprintf(stdout, "Warning: SetModified() in local mode. player:%d type: %d \n", obj->player, obj->type);
  ...
  if (mode == SENDOBJDEAD && obj->modified != SENDOBJSEND && obj->type == ASTEROID)
  {
    if (debugcomm1)
    {
      fprintf(stdout, "SetModified():ASTEROID Debug Warning forbidden. trying to change mode (%d) %d to %d\n", obj->id, obj->modified, mode);
    }
  }
#endif
  ...
  if (mode == SENDOBJNEW)
  { /* SENDOBJNEW only in NewObj() */
#if DEBUG
    if (debugcomm1)
    {
      fprintf(stdout, "SetModified(): Debug Warning forbidden.trying to change mode (%d) %d to %d\n", obj->id, obj->modified, mode);
    }
#endif
    return (2);
  }
  if (0 && mode == SENDOBJNOTSEND)
  { /* SENDOBJNOTSEND only in NewObj() */
#if DEBUG
    if (debugcomm1)
    {
      fprintf(stdout, "SetModified(): Debug Warning forbidden. trying to change mode (%d) %d to %d\n", obj->id, obj->modified, mode);
  ...
  if (0 && mode == SENDOBJDEAD)
  { /* SENDOBJDEAD only in exceptions below */
#if DEBUG
    if (debugcomm1)
    {
      fprintf(stdout, "SetModified(): Debug Warning forbidden.trying to change mode (%d) %d to %d\n", obj->id, obj->modified, mode);
    ...
    case PLANET:
      switch (obj->modified)
      {
        case SENDOBJUNMOD:
          switch (mode)
          {
            ...
            default:
              ...
              if (debugcomm1)
              {
                fprintf(stdout, "Debug Warning 0 SetModified()PLANET:trying to change mode (%d) %d to %d\n", obj->id, obj->modified, mode);
              }
#endif
              break;
          }
        ...
        case SENDOBJPLANET:
          switch (mode)
          {
            ...
            default:
              ...
              if (debugcomm1)
              {
                fprintf(stdout, "Debug Warning 1 SetModified()PLANET:trying to change mode (%d) %d to %d\n", obj->id, obj->modified, mode);
              }
#endif
              break;
          }
        ...
        case SENDOBJSEND:
          switch (mode)
          {
            ...
            default:
              ...
              if (debugcomm1)
              {
                fprintf(stdout, "Debug Warning 2 SetModified()PLANET:trying to change mode (%d) %d to %d\n", obj->id, obj->modified, mode);
              }
#endif
              break;
          }
        ...
        default:
          ...
          if (debugcomm1)
          {
            fprintf(stdout, "Debug Warning 3 SetModified()PLANET:state not allowed (%d) %d to %d\n", obj->id, obj->modified, mode);
          }
#endif
          break;
      }
          ...
          switch (mode)
          {
            ...
            case SENDOBJKILL:
              ...
              if (obj->type != PROJECTILE)
              {
                if (debugcomm1)
                {
                  fprintf(stdout, "SetModified(): id %d (%d) from NEW to DEAD\n", obj->id, obj->type);
                }
              }
#endif
            ...
            default:
              ...
              if (debugcomm1)
              {
                fprintf(stdout, "SetModified():Debug Warning 1. PROJECTILE:trying to change mode (%d) %d to %d\n", obj->id, obj->modified, mode);
              }
#endif
              return (2);
              break;
        ...
        case SENDOBJSEND:
          switch (mode)
          {
            ...
            case SENDOBJMOD0:
              if (obj->type == PROJECTILE)
              {
                ...
                else
                {
                  ...
                  if (debugcomm1)
                  {
                    fprintf(stdout, "Debug Warning 2 SetModified()PROJECTILE:trying to change mode (%d) %d to %d\n", obj->id, obj->modified, mode);
                  }
#endif
                  return (2);
                }
            ...
            default:
              ...
              if (debugcomm1)
              {
                fprintf(stdout, "Debug Warning 3 SetModified()PROJECTILE:trying to change mode (%d) %d to %d\n", obj->id, obj->modified, mode);
              }
#endif
              return (2);
              break;
...
int
SetModifiedAll(struct HeadObjList* lh, int type, int mode, int force)
{
  /*
...
int
CheckModifiedPre(struct HeadObjList* lh, int proc)
{
  /*
...
int
CheckModifiedPost(struct HeadObjList* lh, int proc)
{
  /*
...
void
Setttl(struct HeadObjList* lh, int n)
{
  /*
    version 0.1
          ...
          if ((gcooperative == TRUE && ps[obj->player].control == HUMAN) || genemyknown)
          {
            ...
            { /*  */
              ...
              case 0:                          /* (4r,inf) */
                ...
                if (obj->mode == LANDED && obj->type == SHIP && obj->subtype == TOWER)
                  obj->ttl *= 2;
                break;
              case 1: /* (3r,4r) */
...
void
LoadBuffer(int order, struct Buffer* buffer, int mode)
{
  /*
    version 01
   */
...
int
ServerProcessBuffer(struct Buffer* buffer)
{
...
int
ClientProcessBuffer(struct Buffer* buffer)
{
  struct MessageHeader mess;
  char* buf;
  int nbytes;
...
int
CopyMessHeader2Buffer(struct Buffer* buffer, struct MessageHeader* messh)
{
  int nbytes;
...
int
CopyGlobal2Buffer(struct Buffer* buffer)
{
  /*
...
int
CopyPlayer2Buffer(struct Buffer* buffer, struct Player* player)
{
  int nbytes;
  struct PlayerAll* playerall;
...
int
CopyPlayerMod2Buffer(struct Buffer* buffer, struct Player* player)
{
  int nbytes;
...
int
CopyOrder2Buffer(struct Buffer* buffer, struct Order* order)
{
  int nbytes;
...
int
CopyInt2Buffer(struct Buffer* buffer, int* i)
{
  int nbytes;
...
int
AddObjOrders2Buffer(struct Buffer* buffer, Object* obj)
{
  /*
    version01 (021210)
...
int
NetMess(struct NetMess* mess, int action)
{
  /*
...
int
CopyNetMess2Buffer(struct Buffer* buffer, struct NetMess* mess0)
{
  /*
...
int
AddObjCargo2Buffer(struct Buffer* buffer, Object* obj)
{
  /*
...
int
CopyObjCargofromBuffer(Object* obj0, char* buf0)
{
