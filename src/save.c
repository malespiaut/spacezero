...
char*
CreateSaveFile(int server, int client)
{
...
int
ExecSave(struct HeadObjList lh, char* nom)
{
  /*
  ...
  */
  ...
  fprintf(fp, "%d %d %d %d %d %d %d %d %d %d %d %d\n", GameParametres(GET, GWIDTH, 0), GameParametres(GET, GHEIGHT, 0), GameParametres(GET, GNET, 0), GameParametres(GET, GNGALAXIES, 0), GameParametres(GET, GNPLAYERS, 0), GameParametres(GET, GNPLANETS, 0), GameParametres(GET, GKPLANETS, 0), GameParametres(GET, GPIRATES, 0), GameParametres(GET, GENEMYKNOWN, 0), GameParametres(GET, GCOOPERATIVE, 0), GameParametres(GET, GCOMPCOOPERATIVE, 0), GameParametres(GET, GQUEEN, 0));

  fprintf(fp, "%d %d %d %d %d %d %d %d\n", GameParametres(GET, GULX, 0), GameParametres(GET, GULY, 0), actual_player0, record, nav_mode, GetTime(), g_objid, g_projid);
  ...
  fprintf(fp, "%d %d %d %d %d %d %d %d %d %d %d\n", glocal.actual_player, glocal.g_objid, glocal.g_projid, glocal.ship_c, glocal.cv, glocal.habitat_type, glocal.habitat_id, glocal.fobj[0], glocal.fobj[1], glocal.fobj[2], glocal.fobj[3]);
  fprintf(fp, "%d %d %d %d %d %d %d %d %d %d %d\n", gclient.actual_player, gclient.g_objid, gclient.g_projid, gclient.ship_c, gclient.cv, gclient.habitat_type, gclient.habitat_id, gclient.fobj[0], gclient.fobj[1], gclient.fobj[2], gclient.fobj[3]);
  ...
  for (i = 0; i < GameParametres(GET, GNPLAYERS, 0) + 2; i++)
  {
    ...
    nsectors = players[i].ksectors.n;
    fprintf(fp, "%s %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %f %d %d %d %d %d %d %f %d %d ", players[i].playername, players[i].id, players[i].status, players[i].pid, players[i].proc, players[i].control, players[i].team, players[i].profile, players[i].strategy, players[i].gmaxlevel, players[i].maxlevel, players[i].color, players[i].cv, players[i].nplanets, players[i].nships, players[i].nbuildships, players[i].gold, players[i].lastaction, players[i].ndeaths, players[i].nkills, players[i].points, players[i].goldships, players[i].goldupdates, players[i].goldweapon, nplanets, nsectors);
  ...
  while (ls != NULL)
  {
    ...
    fprintf(fp, "%d %d %d ", ls->obj->cargo.capacity, ls->obj->cargo.mass, ls->obj->cargo.n);
    fprintf(fp, "\n");
    ls = ls->next;
  }
...
ExecLoad(char* nom)
{
  ...
  */
  ...
  if (fscanf(fp, "%d%d%d%d%d%d%d%d", &ulx, &uly, &actual_player0, &record, &nav_mode, &gtime, &g_objid, &g_projid) != 8)
  {
    perror("fscanf");
    exit(-1);
  }
  ...
  if (fscanf(fp, "%d%d%d%d%d%d%d%d%d%d%d", &glocal.actual_player, &glocal.g_objid, &glocal.g_projid, &glocal.ship_c, &glocal.cv, &glocal.habitat_type, &glocal.habitat_id, &fobj[0], &fobj[1], &fobj[2], &fobj[3]) != 11)
  {
    perror("fscanf");
    exit(-1);
  }
  if (fscanf(fp, "%d%d%d%d%d%d%d%d%d%d%d", &gremote.actual_player, &gremote.g_objid, &gremote.g_projid, &gremote.ship_c, &gremote.cv, &gremote.habitat_type, &gremote.habitat_id, &gremote.fobj[0], &gremote.fobj[1], &gremote.fobj[2], &gremote.fobj[3]) != 11)
  {
    perror("fscanf");
    exit(-1);
  }
  ...
  for (i = 0; i < GameParametres(GET, GNPLAYERS, 0) + 2; i++)
  {
    ...
    if (fscanf(fp, "%128s%hd%d%d%hd%hd%hd%hd%hd%hd%hd%d%d%d%d%d%f%d%d%d%d%d%d%f%d%d", players[i].playername, &players[i].id, &players[i].status, &players[i].pid, &players[i].proc, &players[i].control, &players[i].team, &players[i].profile, &players[i].strategy, &players[i].gmaxlevel, &players[i].maxlevel, &players[i].color, &players[i].cv, &players[i].nplanets, &players[i].nships, &players[i].nbuildships, &players[i].gold, &players[i].lastaction, &players[i].ndeaths, &players[i].nkills, &players[i].points, &players[i].goldships, &players[i].goldupdates, &players[i].goldweapon, &nkp, &nks) != 26)
    {
      perror("fscanf");
      exit(-1);
    }
  ...
  for (i = 0; i < num_objs; i++)
  {
    ...
    id = g_objid;
    projid = g_projid;
    nobj = NewObj(SHIP, SHIP0, obj.x, obj.y, obj.vx, obj.vy, CANNON0, ENGINE0, 0, NULL, NULL); /* HEREIN */
    if (nobj == NULL)
    {
    ...
    if (fscanf(fp, "%d%d%d", &nobj->cargo.capacity, &nobj->cargo.mass, &nobj->cargo.n) != 3)
    {
      perror("fscanf");
      exit(-1);
    }
...
FprintfObj(FILE* fp, Object* obj)
{
  ...
  */
  ...
  fprintf(fp, "%d %d %d %d %s %hd %hd %hd %hd %g %d %d %d %d %d %d %d %d %d %d %g %d %hd %hd %hd %hd %hd ", obj->id, obj->pid, obj->oriid, obj->destid, obj->name, obj->player, obj->type, obj->subtype, obj->level, obj->experience, obj->kills, obj->ntravels, obj->durable, obj->visible, obj->radar, obj->cloak, obj->mass, obj->items, obj->cargo.capacity, obj->radio, obj->cost, obj->damage, obj->ai, modified, ttl, obj->habitat, obj->mode);

  fprintf(fp, "%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g ", obj->x, obj->y, obj->x0, obj->y0, obj->vx, obj->vy, obj->fx, obj->fy, obj->fx0, obj->fy0, obj->a, obj->ang_v, obj->ang_a, obj->accel, obj->gas, obj->gas_max, obj->life, obj->shield, obj->state);

  fprintf(fp, "%g %d %d %d ", obj->dest_r2, obj->sw, obj->trace, obj->norder);

  fprintf(fp, "%d %d %d %d ", obj->actorder.priority, obj->actorder.id, obj->actorder.time, obj->actorder.g_time);
  fprintf(fp, "%f %f %f %f %f %f %f %f ", obj->actorder.a, obj->actorder.b, obj->actorder.c, obj->actorder.d, obj->actorder.e, obj->actorder.f, obj->actorder.g, obj->actorder.h);

  fprintf(fp, "%d %d %d %d %d %g %g %g %g %d ", obj->engine.type, obj->engine.a_max, obj->engine.a, obj->engine.v_max, obj->engine.v2_max, obj->engine.ang_a, obj->engine.ang_a_max, obj->engine.ang_v_max, obj->engine.gascost, obj->engine.mass);

  fprintf(fp, "%d %d %d %d %d %d %d ", obj->weapon0.type, obj->weapon0.rate, obj->weapon0.nshots, obj->weapon0.cont1, obj->weapon0.mass, obj->weapon0.n, obj->weapon0.max_n);

  fprintf(fp, "%d %d %d %d %d %d %d %f ", obj->weapon0.projectile.type, obj->weapon0.projectile.durable, obj->weapon0.projectile.life, obj->weapon0.projectile.damage, obj->weapon0.projectile.max_vel, obj->weapon0.projectile.mass, obj->weapon0.projectile.gascost, obj->weapon0.projectile.unitcost);

  fprintf(fp, "%d %d %d %d %d %d %d ", obj->weapon1.type, obj->weapon1.rate, obj->weapon1.nshots, obj->weapon1.cont1, obj->weapon1.mass, obj->weapon1.n, obj->weapon1.max_n);

  fprintf(fp, "%d %d %d %d %d %d %d %f ", obj->weapon1.projectile.type, obj->weapon1.projectile.durable, obj->weapon1.projectile.life, obj->weapon1.projectile.damage, obj->weapon1.projectile.max_vel, obj->weapon1.projectile.mass, obj->weapon1.projectile.gascost, obj->weapon1.projectile.unitcost);

  fprintf(fp, "%d %d %d %d %d %d %d ", obj->weapon2.type, obj->weapon2.rate, obj->weapon2.nshots, obj->weapon2.cont1, obj->weapon2.mass, obj->weapon2.n, obj->weapon2.max_n);
  fprintf(fp, "%d %d %d %d %d %d %d %f ", obj->weapon2.projectile.type, obj->weapon2.projectile.durable, obj->weapon2.projectile.life, obj->weapon2.projectile.damage, obj->weapon2.projectile.max_vel, obj->weapon2.projectile.mass, obj->weapon2.projectile.gascost, obj->weapon2.projectile.unitcost);
...
int
FprintfPlanet(FILE* fp, Object* obj)
{
  /*
  ...
  */
  ...
  fprintf(fp, "%d %d %g %g %g ", planet->x, planet->y, planet->r, planet->reggold, planet->gold);
...
int
FscanfObj(FILE* fp, Object* obj, struct ObjTable* tbl)
{
  /*
  ...
  */
  ...
  if (fscanf(fp, "%d%d%d%d%16s%hd%hd%hd%hd%f%d%d%d%d%d%d%d%d%d%d%f%d%hd%hd%hd%hd%hd", &obj->id, &obj->pid, &obj->oriid, &obj->destid, obj->name, &obj->player, &obj->type, &obj->subtype, &obj->level, &obj->experience, &obj->kills, &obj->ntravels, &obj->durable, &obj->visible, &obj->radar, &obj->cloak, &obj->mass, &obj->items, &obj->cargo.capacity, &obj->radio, &obj->cost, &obj->damage, &obj->ai, &modified, &obj->ttl, &obj->habitat, &obj->mode) != 27)
  {
    perror("fscanf");
    exit(-1);
  }
  ...
  if (fscanf(fp, "%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f", &obj->x, &obj->y, &obj->x0, &obj->y0, &obj->vx, &obj->vy, &obj->fx, &obj->fy, &obj->fx0, &obj->fy0, &obj->a, &obj->ang_v, &obj->ang_a, &obj->accel, &obj->gas, &obj->gas_max, &obj->life, &obj->shield, &obj->state) != 19)
  {
    perror("fscanf");
    exit(-1);
  }

  if (fscanf(fp, "%f%d%hd%d", &obj->dest_r2, &obj->sw, &obj->trace, &obj->norder) != 4)
  {
    perror("fscanf");
    exit(-1);
  }

  if (fscanf(fp, "%d%d%d%d", &obj->actorder.priority, &obj->actorder.id, &obj->actorder.time, &obj->actorder.g_time) != 4)
  {
    perror("fscanf");
    exit(-1);
  }
  if (fscanf(fp, "%f%f%f%f%f%f%f%f", &obj->actorder.a, &obj->actorder.b, &obj->actorder.c, &obj->actorder.d, &obj->actorder.e, &obj->actorder.f, &obj->actorder.g, &obj->actorder.h) != 8)
  {
    perror("fscanf");
    exit(-1);
  }

  if (fscanf(fp, "%d%d%d%d%d%f%f%f%f%d", &obj->engine.type, &obj->engine.a_max, &obj->engine.a, &obj->engine.v_max, &obj->engine.v2_max, &obj->engine.ang_a, &obj->engine.ang_a_max, &obj->engine.ang_v_max, &obj->engine.gascost, &obj->engine.mass) != 10)
  {
    perror("fscanf");
    exit(-1);
  }

  if (fscanf(fp, "%d%d%d%d%d%d%d", &obj->weapon0.type, &obj->weapon0.rate, &obj->weapon0.nshots, &obj->weapon0.cont1, &obj->weapon0.mass, &obj->weapon0.n, &obj->weapon0.max_n) != 7)
  {
    perror("fscanf");
    exit(-1);
  }

  if (fscanf(fp, "%d%d%d%d%d%d%d%f", &obj->weapon0.projectile.type, &obj->weapon0.projectile.durable, &obj->weapon0.projectile.life, &obj->weapon0.projectile.damage, &obj->weapon0.projectile.max_vel, &obj->weapon0.projectile.mass, &obj->weapon0.projectile.gascost, &obj->weapon0.projectile.unitcost) != 8)
  {
    perror("fscanf");
    exit(-1);
  }

  if (fscanf(fp, "%d%d%d%d%d%d%d", &obj->weapon1.type, &obj->weapon1.rate, &obj->weapon1.nshots, &obj->weapon1.cont1, &obj->weapon1.mass, &obj->weapon1.n, &obj->weapon1.max_n) != 7)
  {
    perror("fscanf");
    exit(-1);
  }
  if (fscanf(fp, "%d%d%d%d%d%d%d%f", &obj->weapon1.projectile.type, &obj->weapon1.projectile.durable, &obj->weapon1.projectile.life, &obj->weapon1.projectile.damage, &obj->weapon1.projectile.max_vel, &obj->weapon1.projectile.mass, &obj->weapon1.projectile.gascost, &obj->weapon1.projectile.unitcost) != 8)
  {
    perror("fscanf");
    exit(-1);
  }

  if (fscanf(fp, "%d%d%d%d%d%d%d", &obj->weapon2.type, &obj->weapon2.rate, &obj->weapon2.nshots, &obj->weapon2.cont1, &obj->weapon2.mass, &obj->weapon2.n, &obj->weapon2.max_n) != 7)
  {
    perror("fscanf");
    exit(-1);
  }
  if (fscanf(fp, "%d%d%d%d%d%d%d%f", &obj->weapon2.projectile.type, &obj->weapon2.projectile.durable, &obj->weapon2.projectile.life, &obj->weapon2.projectile.damage, &obj->weapon2.projectile.max_vel, &obj->weapon2.projectile.mass, &obj->weapon2.projectile.gascost, &obj->weapon2.projectile.unitcost) != 8)
  {
    perror("fscanf");
    exit(-1);
  }
...
int
FscanfPlanet(FILE* fp, struct Planet* planet)
{
  /*
  ...
  */
  ...
  if (fscanf(fp, "%d%d%f%f%f", &planet->x, &planet->y, &planet->r, &planet->reggold, &planet->gold) != 5)
  {
    perror("fscanf");
    exit(-1);
  }
...
int
FprintfOrders(FILE* fp, Object* obj)
{
  /*
  ...
  */
  ...
  while (lo != NULL)
  {
    ...
    fprintf(fp, "%g %g %g %g %g %g %g %g %g %g %g %g ", ord->a, ord->b, ord->c, ord->d, ord->e, ord->f, ord->g, ord->h, ord->i, ord->j, ord->k, ord->l);
    i++;
    lo = lo->next;
  }
  if (i != n)
...
int
FscanfOrders(FILE* fp, Object* obj)
{
  /*
  ...
  */
  ...
  for (i = 0; i < n; i++)
  {
    if (fscanf(fp, "%d%d%d%d", &order.priority, &order.id, &order.time, &order.g_time) != 4)
    {
      perror("fscanf");
      exit(-1);
    }
    if (fscanf(fp, "%f%f%f%f%f%f%f%f%f%f%f%f", &order.a, &order.b, &order.c, &order.d, &order.e, &order.f, &order.g, &order.h, &order.i, &order.j, &order.k, &order.l) != 12)
    {
      perror("fscanf");
      exit(-1);
    }
...
void
FprintfCCData(FILE* fp, struct CCDATA* ccdata)
{

  fprintf(fp, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %f\n", ccdata->player, ccdata->time, ccdata->time2, ccdata->nkplanets, ccdata->nplanets, ccdata->ninexplore, ccdata->nenemy, ccdata->nexplorer, ccdata->nfighter, ccdata->npilot, ccdata->ntower, ccdata->ncargo, ccdata->sw, ccdata->war, ccdata->p2a_strength);
...
void
FprintfPlanetInfoList(FILE* fp, struct CCDATA* ccdata)
{
...
void
FprintfPlanetInfo(FILE* fp, struct PlanetInfo* pinfo)
{
  int objid;
...
#endif
  fprintf(fp, "%d %d %d %d %d %d %d %f %f %d\n", objid, pinfo->time, pinfo->nexplorer, pinfo->nfighter, pinfo->npilot, pinfo->ntower, pinfo->ncargo, pinfo->strength, pinfo->strengtha, pinfo->nassigned);
}

void
FscanfCCData(FILE* fp, struct CCDATA* ccdata)
{

  int pld, pw, phl, p2m, p2a;

  if (fscanf(fp, "%d%d%d%d%d%d%d%d%d%d%d%d%d%d%f", &ccdata->player, &ccdata->time, &ccdata->time2, &ccdata->nkplanets, &ccdata->nplanets, &ccdata->ninexplore, &ccdata->nenemy, &ccdata->nexplorer, &ccdata->nfighter, &ccdata->npilot, &ccdata->ntower, &ccdata->ncargo, &ccdata->sw, &ccdata->war, &ccdata->p2a_strength) != 15)
  {
    perror("fscanf");
    exit(-1);
  }
...
void
FscanfPlanetInfoList(FILE* fp, struct CCDATA* ccdata)
{
  struct PlanetInfo pinfo;
  int i;
  int n = 0;
...
void
FscanfPlanetInfo(FILE* fp, struct PlanetInfo* pinfo)
{
  int objid;
  Object* planet;
  if (fscanf(fp, "%d%d%d%d%d%d%d%f%f%d", &objid, &pinfo->time, &pinfo->nexplorer, &pinfo->nfighter, &pinfo->npilot, &pinfo->ntower, &pinfo->ncargo, &pinfo->strength, &pinfo->strengtha, &pinfo->nassigned) != 10)
  {
    perror("fscanf");
    exit(-1);
  }
...
void
SaveParamOptions(char* file, struct Parametres* par)
{
  ...
  fprintf(fp, "%s\n", version);
  fprintf(fp, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n", par->ngalaxies, par->nplanets, par->nplayers, par->ul, par->kplanets, par->sound, par->music, par->soundvol, par->musicvol, par->cooperative, par->compcooperative, par->queen, par->pirates, par->enemyknown, par->port);
...
int
LoadParamOptions(char* file, struct Parametres* par)
{
  ...
  if (fscanf(fp, "%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d", &par->ngalaxies, &par->nplanets, &par->nplayers, &par->ul, &par->kplanets, &par->sound, &par->music, &par->soundvol, &par->musicvol, &par->cooperative, &par->compcooperative, &par->queen, &par->pirates, &par->enemyknown, &par->port) != 15)
  {
    perror("fscanf");
    exit(-1);
  }
...
void
PrintParamOptions(struct Parametres* par)
{
...
void
SaveRecord(char* file, struct Player* players, int record)
{
  int i, j;
  FILE* fp;
