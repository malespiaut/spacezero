...
void
aimissile(struct HeadObjList* lhobjs, Object* obj)
{
...
void
ai(struct HeadObjList* lhobjs, Object* obj, int act_player)
{
  /*
    main ai subroutine
  ...
  */
  ...
  if (obj->cdata->obj[0] != NULL || (time - obj->cdata->td2[0]) > 50 + obj->id % 20)
  {
    ...
    if (nobjs[0].obj != NULL)
    {
      if (nobjs[0].obj->type == SHIP)
      { /* take off to intercept */
        d2min *= 2;
      }

      if (nobjs[0].obj->type == ASTEROID && control == COMPUTER)
      { /* HERE send to asteroids from ControlCenter() */
        d2min = 25 * obj->radar * obj->radar;
      }

      if (nobjs[0].d2 > d2min)
      { /* nearest ship must be in radar range */
        nobjs[0].obj = NULL;
        nobjs[0].d2 = -1;
      }
    }
    ...
    switch (ship_enemy->type)
    {
      case SHIP:
        if (obj->habitat == H_PLANET && (players[obj->player].team == players[obj->in->player].team))
        {
          ...
          {
            snprintf(text, MAXTEXTLEN, "%s %d %s!!", GetLocale(L_PLANET), objt->pid, GetLocale(L_UNDERATTACK));
            value = 3;
          }
        }
        else
        ...
        break;
      case ASTEROID:
        snprintf(text, MAXTEXTLEN, "(%c %d) %s", Type(obj), obj->pid, GetLocale(L_ASTEROIDSNEAR));
        value = 0;
        break;
      default:
        break;
  ...
  ********************/
  ...
  if (mainord != NULL)
  {
    if (mainord->id == RETREAT)
    { /* in retreat, when reach destiny change the order */
      int sw = 0;
      if ((int)mainord->c != -1)
      {
        if (obj->habitat == H_PLANET)
        {
          if (obj->in->id == (int)mainord->c)
          { /* dest reached */
      ...
      if (sw == 1)
      {                                    /* in retreat, when reach a planet change order by goto */
    ...
    switch (danger)
    {
      case 0: /* no danger, execute main order */
        if (mainord != NULL)
        {
          ...
          if (mainord->id == EXPLORE && (int)mainord->h == 1)
          { /* select sector */
            int a, b;
          ...
          switch (mainord->id)
          {
            ...
            case EXPLORE:
                  ...
                  if (obj->habitat == H_PLANET)
                  {
                    if (obj->in->id == objt->id)
                    {
                      if ((int)obj->player == act_player && (int)mainord->h == 0)
                      {
                        ...
                        snprintf(text, MAXTEXTLEN, "(%c %d) %s %d", Type(obj), obj->pid, GetLocale(L_ARRIVEDTO), obj->in->id);
              ...
              if ((int)mainord->c == -1)
              { /* destination is a sector */
                if (fabs(obj->x - mainord->a) < 400 && fabs(obj->y - mainord->b) < 400)
                {
                  if ((int)obj->player == act_player && (int)mainord->h == 0 && mainord->id != EXPLORE)
                  {
                    snprintf(text, MAXTEXTLEN, "(%c %d) %s %d %d", Type(obj), obj->pid, GetLocale(L_ARRIVEDTO), (int)(mainord->a / SECTORSIZE) - (mainord->a < 0.0), (int)(mainord->b / SECTORSIZE) - (mainord->b < 0.0));
                    if (!Add2TextMessageList(&listheadtext, text, obj->id, obj->player, 0, 100, 0))
                    {
                      Add2CharListWindow(&gameloglist, text, 1, &windowgamelog);
                    }
...
int
HigherPoint(struct Planet* planet)
{
  /*
...
CreateCCData(void)
{
  ...
  { /* HERE TODO include ccdata in player*/
    ccdatap[i].player = i;
...
Object*
CCUpgrade(struct HeadObjList* lhobjs, struct Player* player)
{
  /*
    version 00 18Nov10
    Keep simple:
...
int
CCBuy(struct HeadObjList* lhobjs, struct CCDATA* ccdata, struct Player* player, int* planetid)
{
  /*
    version 01 29Oct10
    Keep simple:
  ...
  if (player->id != ccdata->planethighresource->player)
  { /* HERE this happens (maybe if in the meanwhile change of owner) */
  ...
  if (!(time % 100))
  {
    printf("CCDATA:ncargo %d: %d %d\n", player->id, ccdata->ncargo, ccdata->nplanets / (ccdata->ncargo + 1));
  }
#endif
...
void
ControlCenter(struct HeadObjList* lhobjs, struct Player* player)
{
  /*
    version 08 311012
  ...
  */
  ...
  {
    printf("WAR player: %d war:%d p1:%p\n", player->id, ccdata->war, (void*)pinfo1);
  }
#endif
    ...
    switch (actord->id)
    {
      case NOTHING:
        switch (obj->mode)
        {
          ...
          case NAV: /* actual order=NOTHING */
            ...
            if (obj->gas > 0.25 * obj->gas_max && obj->cdata->a == 0)
            {                                                                     /* return or explore*/
      ...
      case EXPLORE:
        ...
        if (sw)
        {
          ...
          { /*if there no known planets, no own planets: explore */
            /* */
            if (!(sw & 2))
            { /* has gas */
              ord.priority = 1;
...
Object*
ObjFromPlanet(struct HeadObjList* lhobjs, int planetid, int player)
{
  /*
...
Object*
ObjMinExperience(struct HeadObjList* lhobjs, int player)
{
  /*
...
Object*
Coordinates(struct HeadObjList* lhobjs, int id, float* x, float* y)
{
  /*
...
int
Risk_08(struct HeadObjList* lhobjs, Object* obj)
{
  /*
...
int
Risk(struct HeadObjList* lhobjs, Object* obj)
{
  /*
...
ChooseWeapon(Object* obj)
{
  ...
  */
  ...
  for (j = 0; j < 3; j++)
  {
    ...
    if (obj->dest_r2 < a[0])
      b[0] = 1;
    if (weapon->n > a[1] * weapon->max_n)
      b[1] = 1;
    if (obj->gas > a[2])
      b[2] = 1;
    if (weapon->cont1 != 0)
      b[4] = 0; /* not avalaible temporaly */
    if (weapon->n == 0)
      b[4] = 0; /* not avalaible, empty */
    ...
    switch (weapon->projectile.type)
    {
      ...
      case LASERBEAM:
        ...
        if (obj->gas > a[2][j])
          b[2][j] = 1;
        if (weapon->cont1 != 0)
          b[4][j] = 0; /* not avalaible temporaly */
        if (weapon->n == 0)
          b[4][j] = 0; /* not avalaible, empty */
        break;
      default:
        break;
    }
...
int
FireCannon(struct HeadObjList* lhobjs, Object* obj1, Object* obj2)
{
  /*
    version 0.1 040810
   */
  ...
  switch (obj1->weapon->type)
  {
    ...
    case CANNON6:
      ...
      for (i = 0; i < n; i++)
      {
        ...
        obj = NewObj(PROJECTILE, obj1->weapon->projectile.type, obj1->x + r * Cos(ang), obj1->y + r * Sin(ang), vp * Cos(ang) + obj1->vx, vp * Sin(ang) + obj1->vy, CANNON0, ENGINE0, obj1->player, obj1, obj1->in);
        if (obj != NULL)
        {
          ...
          if (enemies == 0)
          { /* if there are no enemies don't send */
    ...
    case CANNON7: /* not finished, not used */
      ...
      {
        ang = obj1->a + ang0 + 0.04 * (0.5 * (1 - n) + i);
        obj = NewObj(PROJECTILE, obj1->weapon->projectile.type, obj1->x + r * Cos(ang), obj1->y + r * Sin(ang), vp * Cos(ang), vp * Sin(ang), CANNON0, ENGINE0, obj1->player, obj1, obj1->in);
        if (obj != NULL)
        {
          if (obj1->habitat == H_PLANET && 0)
          {
    ...
    case CANNON8: /* missile */
      ang = obj1->a;
      obj = NewObj(PROJECTILE, obj1->weapon->projectile.type, obj1->x + r * Cos(ang), obj1->y + r * Sin(ang), vp * Cos(ang) + obj1->vx, vp * Sin(ang) + obj1->vy, CANNON0, ENGINE2, obj1->player, obj1, obj1->in);
      if (obj != NULL)
      {
        if (obj1->habitat == H_PLANET && 0)
        {
    ...
    case CANNON10:
      ...
      if (1)
      {
        obj = NewObj(PROJECTILE, obj1->weapon->projectile.type, obj1->x + 0.5 * r * Cos(ang), obj1->y + 0.5 * r * Sin(ang), vp * Cos(ang) + obj1->vx, vp * Sin(ang) + obj1->vy, CANNON0, ENGINE0, obj1->player, obj1, obj1->in);
...
void
Play(Object* obj, int sid, float vol)
{
  /*
...
struct Order*
ReadOrder(struct Order* order0, Object* obj, int mode)
{
  /*
  ...
  switch (mode)
  {
    case MAXPRIORITY:
      ...
      while (lo != NULL)
      {
        ...
        { /* read the first of max priority */
          lopm = lop;
          lom = lo;
...
int
AddOrder(Object* obj, struct Order* order)
{
  /*
...
int
CountPlanetInfoList(struct CCDATA* ccdata)
{
  /*
...
void
CreatePirates(struct HeadObjList* lhobjs, int n, float x0, float y0, float level)
{
  /*
    Create some pirates
   */
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (i == 0)
    { /* EXPLORER*/
      obj = NewObj(SHIP, EXPLORER, x0 + x, y0 + y, 0, 0, CANNON3, ENGINE3, GameParametres(GET, GNPLAYERS, 0) + 1, NULL, NULL);
    }
    else
    { /* FIGHTER*/
      obj = NewObj(SHIP, FIGHTER, x0 + x, y0 + y, 0, 0, CANNON4, ENGINE4, GameParametres(GET, GNPLAYERS, 0) + 1, NULL, NULL);
    }
    if (obj != NULL)
    {
      obj->a = PI / 2;
...
void
CreateAsteroids(struct HeadObjList* lhobjs, int n, float x0, float y0)
{
  /*
  ...
  */
  ...
  for (i = 0; i < n; i++)
  {
    ...
    obj = NewObj(ASTEROID, size + 1, x0 + x, y + y0, vx, vy, CANNON0, ENGINE0, 0, NULL, NULL);
    if (obj != NULL)
    {
      obj->ai = 0;
      obj->in = NULL;
...
void
GetInformation(struct Player* p1, struct Player* p2, Object* obj)
{
  /*
...
int
OtherProc(struct HeadObjList* lh, int p, Object* obj0)
{
  /*
    version 0.2
...
void
CalcCCInfo(struct HeadObjList* lhobjs, struct HeadObjList* lhkplanets, int player, struct CCDATA* ccdata)
{
...
int
AddobjCCData(struct CCDATA* ccdata, Object* obj)
{
  /*
  ...
  while (pinfo != NULL)
  {
    ...
    if (obj->habitat == H_PLANET)
    {
      if (pinfo->planet->id == obj->in->id)
      {
        ...
        {
          if (obj->level >= obj->in->level - 2)
          { /* don't include learning ships*/
            float factor = 0;
...
void
CalcCCPlanetStrength(int player, struct CCDATA* ccdata)
{
  /*
  ...
  if (debugwar)
  {
    ...
    if (ccdata->planetweak != NULL)
    {
      printf("pweak: %d ", ccdata->planetweak->id);
    }
    else
    {
      printf("pweak: NULL ");
...
int
GetCCPlanetInfo(struct CCDATA* ccdata, int pid, int info)
{
  struct PlanetInfo* pinfo;
...
void
PrintCCPlanetInfo(struct CCDATA* ccdata)
{
  struct PlanetInfo* pinfo;
  ...
  while (pinfo != NULL)
  {
    ...
    {
      printf("player:%d planet: %d ne:%d nf:%d nt:%d na:%d\n\t str: %f stra:%f\n", ccdata->player, pinfo->planet->id, pinfo->nexplorer, pinfo->nfighter, pinfo->ntower, pinfo->nassigned, pinfo->strength, pinfo->strengtha);
    }
#endif
...
void
DestroyCCPlanetInfo(struct CCDATA* ccdata)
{
...
int
NearestCCPlanets(struct CCDATA* ccdata, Object* obj, int status, struct NearObject* objs)
{
  /*
...
int
WarCCPlanets(struct Player* player, struct CCDATA* ccdata)
{
  /*
    version 02 Oct2012
    ...
    case PLAYERSTRATSTRONG: /* from strongest to nearest */
      ...
      while (pinfo != NULL)
      {
        ...
        else
        {
          ...
          {
            if (100 * Random(-1) < 20)
            { /* HERE must depend on number of planets*/
              planet = pinfo->planet;
            }
          }
        }
...
int
DecideWar(struct Player* player, struct CCDATA* ccdata)
{
  /*
    Decide if go to War
   */
...
struct PlanetInfo*
GetPlanetInfo(struct CCDATA* ccdata, Object* planet)
{
  /*
...
Object*
GetNearPlanet(struct CCDATA* ccdata, Object* planet1, int pstate)
{
  /*
...
int
IsInCCList(struct CCDATA* ccdata, Object* planet)
{
  /*
    returns:
...
struct PlanetInfo*
AddNewPlanet2CCData(struct CCDATA* ccdata, Object* planet)
{
  /*
...
int
AddPlanetInfo2CCData(struct CCDATA* ccdata, struct PlanetInfo* pinfo0)
{
  /*
    Add a new planet to ccdata list
...
int
ResetPlanetCCInfo(struct CCDATA* ccdata, Object* planet)
{
  /*
    returns:
...
struct PlanetInfo*
CalcEnemyPlanetInfo(struct HeadObjList* lhobjs, struct CCDATA* ccdata, Object* obj)
{
  /*
  ...
  if (retpinfo != NULL)
  {
    ...
    if (debugwar)
    {
      ...
      printf("Planet %d info: str: %f nf: %d (%d,%d)\n",
             ...
             retpinfo->strength,
             retpinfo->nfighter,
             players[planet->player].team,
             players[ccdata->player].team);
      printf("********************\n");
    }
#endif
  }
...
int
UpdateEnemyPlanetInfo(struct HeadObjList* lhobjs, struct CCDATA* ccdata, int player)
{
  /*
...
int
CountAssignedCCPlanetInfo(struct HeadObjList* lhobjs, struct CCDATA* ccdata, Object* planet)
{
  /*
...
struct PlanetInfo*
War(struct HeadObjList* lhobjs, struct Player* player, struct CCDATA* ccdata)
{
  /*
    returns :
    planet2meet
*/
  ...
  if (ccdata->war >= 2)
  {
    ...
    if (players[ccdata->planet2attack->player].team == players[player->id].team)
    { /* planet conquered */
    ...
    if (players[ccdata->planet2meet->player].team != players[player->id].team)
    { /* origin planet lost */
#if DEBUG
      if (debugwar)
      {
  ...
  if (ccdata->war && debugwar)
  {
    printf("INWAR player %d war:%d p1 %p p2%p\n", player->id, ccdata->war, (void*)pinfo1, (void*)pinfo2);
  }
#endif
  ...
  switch (ccdata->war)
  {
...
#if TEST
      if (ccdata->war > 0)
        printf("player %d starting WAR %d %d\n", ccdata->player, GetTime(), ccdata->nenemy);
#endif
      break;
    case 1:
      /* Choose planet to attack and from  */
      if (WarCCPlanets(player, ccdata) != 1)
      { /* HERE TODO review this for differents values of 1 */
#if TEST
...
#if TEST
        printf("player %d entering phase 2\n", ccdata->player);
#endif
      }
    ...
    case 2:
      ...
      if (pinfo1 == NULL)
      {
        ...
        ccdata->war = 0;
#if TEST
        printf("player %d resetting 1 WAR\n", ccdata->player);
#endif
        return (NULL);
      }
      ...
      if (1.2 * ccdata->p2a_strength < pinfo2->strength)
      {
        ...
        ccdata->war = 0;
#if TEST
        printf("player %d resetting 2 WAR (dest stronger)\n", ccdata->player);
#endif
        return (NULL);
      }
      ...
      if ((pinfo1)->strengtha > nf2a)
      {
        ...
        ccdata->war = 3;
#if TEST
        printf("player %d entering phase 3\n", ccdata->player);
        printf("player: %d war: %d time: %d\n", ccdata->player, ccdata->war, GetTime());
        printf("\t FROM %d: nf: %d st2a: % f nf2a:%d\n",
               ccdata->planet2meet->id,
               (pinfo1)->nfighter,
    ...
    case 3:
      ...
      if (pinfo2 == NULL)
      {
        ...
        ccdata->war = 0;
#if TEST
        printf("player %d resetting 3 (attack NULL) WAR\n", ccdata->player);
#endif
        return (NULL);
      }
  ...
  if (ccdata->war)
  {
    if (debugwar && !(cont % 200))
    {
      ...
      if (ccdata->planet2meet != NULL && ccdata->planet2attack != NULL)
      {
        ...
        printf("\tplanet2meet %d planet2attack %d (%d)\n", ccdata->planet2meet->id, ccdata->planet2attack->id, strg);
      }
    }
  }
#endif
...
int
BuyorUpgrade(struct HeadObjList* lhobjs, struct Player* player, struct CCDATA* ccdata)
{
  /*
...
*/
  ...
  np = (player->nplanets);
  if (np == 0)
    return (0); /* HERE this doesn't allow pilots to buy when there are no planets */
...
void
SendPlanetInfo2Allies(int pid, struct PlanetInfo* pinfo)
{
  int i;
...
int
ExecGoto(struct HeadObjList* lhobjs, Object* obj, struct Order* ord)
{
  /*
     version 01
  ...
  */
  ...
  switch ((int)ord->d)
  {
    case PLANET:
      ...
      {
        if (obj == cv)
          printf("%f %d\n", d02, obj->engine.a_max);
      }
#endif

      if (d2 < d02)
      { /* ori 40000 objetive reached. */
        ExecBrake(obj, 4);
        return (0);
      }
      break;
...
#if DEBUG
  if (debugexec && obj == cv)
    printf("\tacceleration in goto ret:%d bc: %f factor:%f\n", ret, bc, factor);
#endif
...
void
ExecLand(Object* obj, struct Order* ord)
{
  /*
    version 02.Dec 2012
...
void
ExecAttack(struct HeadObjList* lhobjs, Object* obj, struct Order* ord, struct Order* morder, float d2)
{
  /*
  ...
  */
...
#if DEBUG
        if (debugexec && obj == cv)
          printf("\tExecattack retreat c:%f id:%d\n", c, obj->dest->id);
#endif
        if (ExecGoto(lhobjs, obj, morder) == 1)
        {
#if DEBUG
          if (debugexec && obj == cv)
            printf("\tExecattack retreat execgoto: 1 \n");
#endif
          if (!ExecTurn(obj, c))
          {               /* turn to enemy */
...
#if DEBUG
          if (debugexec && obj == cv)
            printf("\tExecattack retreat execgoto: 0 \n");
#endif
          return;
        }
        break;
  ...
  if (obj->habitat == H_PLANET && obj->engine.a_max && obj->mode != LANDED)
  {
    ...
    if (obj->y < ord->b + (.35 + .10 * obj->in->mass / MAXPLANETMASS) * LYFACTOR)
    { /* refered to higher planet point */
      ord->time = 0;
      ...
      if (fabs(ib) < 0.5)
      { /**obj->engine.ang_a*DT*DT*(100./obj->mass))  */
        obj->ang_a = 0;
      }
...
#if DEBUG
    if (debugexec && obj == cv)
      printf("\texecturn in attack 2 a: %f c: %f\n", obj->a, c);
#endif
    if (ExecTurn(obj, c))
    {
      obj->a = c;
    ...
    case SHIP:
      ...
      switch (obj->habitat)
      {
        ...
        case H_SPACE:
          ...
          if (fabs(ab) < .1 && obj->engine.v2_max - (obj->vx * obj->vx + obj->vy * obj->vy) < 0.025 * obj->engine.v2_max)
          { /* FIX in 0.84 */
            obj->accel = 0;
            swaccel = 0;
            return;
...
int
ExecOrbit(Object* obj, Object* planet, struct Order* mainord)
{
  float vt, vr;
  float v2;
  float a;
  ...
  if (0 && cv == obj)
  {
    printf("a:%f b:%f c:%f\n", obj->actorder.a, obj->actorder.b, obj->actorder.c);
  }
...
int
ExecTurnAccel(Object* obj, float c, float prec)
{
  float b, ac, bc, v2;
  float minang;
  int ret = 1;
...
ExecTurn(Object* obj, float c)
{
  ...
  */
  ...
  else
  {
    ...
    sgnia = ac > 0 ? -1 : 1;
    if (fabs(obj->ang_v) > fabs(0.28 * ac))
    { /* decelerate */
...
int
ExecStop(Object* obj, float v0, float iv2)
{
  /*
    version 04
...
int
ExecStop2(Object* obj1, Object* obj2, float v0, float iv2)
{
  /*
    version 04
    Stop the ship to a velocity lower than v0 (square).
