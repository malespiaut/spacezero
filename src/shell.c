...
int
Shell(int command, GdkGC* color, GdkFont* font, struct HeadObjList* lhead, struct Player* ps, struct Keys* key, Object** pcv, char* cad)
{
  /*
    version 04
  */
  ...
  if (level == 2)
  {
    ...
    switch (order)
    {
      ...
      case BUY:
        ...
        {
          snprintf(cad, MAXTEXTLEN, "1: EXPLORER(%d)   2: FIGTHER(%d)   3: TOWER(%d)   4: FREIGHTER(%d)   5: SATELLITE(%d)   6: CLOAK(%d)", GetPrice(NULL, EXPLORER, ENGINE3, CANNON3), GetPrice(NULL, FIGHTER, ENGINE4, CANNON4), GetPrice(NULL, TOWER, ENGINE1, CANNON4), GetPrice(NULL, FREIGHTER, ENGINE6, CANNON0), GetPrice(NULL, SATELLITE, ENGINE1, CANNON3), GetPrice(NULL, CLOAKDEVICE, ENGINE0, CANNON0));
        }
        level = 3;
        break;
      case UPGRADE:
        strcpy(cad, "");
        snprintf(cad, MAXTEXTLEN, "%s %s  %d %s", ord, par, (int)(GetPrice(cv, 0, 0, 0)), "   (Esc to cancel)");
        break;
      case EXPLORE:
      case STOP:
      case TAKEOFF:
      ...
      case SELL:
        ...
        if (cv->mode == LANDED)
        {
          snprintf(cad, MAXTEXTLEN, "%s %s  %d %s", ord, par, (int)(0.5 * GetPrice(cv, 0, 0, 0)), "   (Esc to cancel)");
        }
        else
        {
          snprintf(cad, MAXTEXTLEN, "DESTROYING %s%s", par, "   (Esc to cancel)");
        }
        break;
      case WRITE:
        strcpy(par, "");
...
Object*
ExecOrder(struct HeadObjList* lhead, Object* obj, struct Player* ps, int order, char* par)
{
  /*
  ...
  */
  ...
  switch (order)
  {
      ...
      switch (nargs)
      {
        case 1: /* GOTO a planet or a ship */
            ...
            switch (obj_dest->type)
            {
              case PLANET: /* if planet is unknown*/
                if (IsInIntList((ps->kplanets), id1) == 0)
                {
                  ...
                  snprintf(stmess, MAXTEXTLEN, "%s. %d %s.", GetLocale(L_NOTALLOWED), obj_dest->pid, GetLocale(L_PLANETORSPACESHIPUNKNOWN));
                  ShellTitle(1, stmess, NULL, NULL, 0, 0);
                  obj_dest = NULL;
                  /* keys.esc=TRUE; */
                }
                else
                {
                  ...
                  printf("(%c %d) %s %d.\n",
                         ...
                         obj_dest->pid);
                  snprintf(stmess, MAXTEXTLEN, "(%c %d) %s %d.", Type(obj), obj->pid, GetLocale(L_GOINGTOPLANET), obj_dest->pid);
                  ShellTitle(1, stmess, NULL, NULL, 0, 0);
                }
                break;
              case SHIP: /* ship belongs to another player*/
                if (obj_dest->player != obj->player)
                {
                  ...
                  snprintf(stmess, MAXTEXTLEN, "%s. Destiny is an enemy spaceship.", GetLocale(L_NOTALLOWED));
                  ShellTitle(1, stmess, NULL, NULL, 0, 0);
                }
                else
                {
                  ...
                  else
                  {
                    printf("(%c %d) %s %d.\n",
                           ...
                           obj_dest->pid);
                    snprintf(stmess, MAXTEXTLEN, "(%c %d) %s %d.", Type(obj), obj->pid, GetLocale(L_GOINGTOSPACESHIP), obj_dest->pid);
                    ShellTitle(1, stmess, NULL, NULL, 0, 0);
                  }
                }
                break;
          ...
          else
          {
            ...
            printf("\t arg1:(%s) par:(%s)\n", arg1, par);
#endif
            snprintf(stmess, MAXTEXTLEN, "%s. (%s) %s.", GetLocale(L_NOTALLOWED), arg1, GetLocale(L_PLANETORSPACESHIPUNKNOWN));
        ...
        case 2: /* GOTO sector    AQUI */
          ...
          snprintf(stmess, MAXTEXTLEN, "(%c %d) %s %d %d.", Type(obj), obj->pid, GetLocale(L_GOINGTOSECTOR), (id1 - SECTORSIZE / 2) / SECTORSIZE, (id2 - SECTORSIZE / 2) / SECTORSIZE);
          ShellTitle(1, stmess, NULL, NULL, 0, 0);
          break;
        default:
    ...
    case EXPLORE:
      ...
      snprintf(stmess, MAXTEXTLEN, "(%c %d) %s.", Type(obj), obj->pid, GetLocale(L_GOINGTOEXPLORE));
    ...
    case SELECT:
      ...
      if (obj_dest != NULL)
      {
        ...
        if (obj_dest != NULL)
        {
          ...
          snprintf(stmess, MAXTEXTLEN, "(%c %d) %s.", Type(obj_dest), obj_dest->pid, GetLocale(L_SELECTED));
          ShellTitle(1, stmess, NULL, NULL, 0, 0);
        }
        else
        {
    ...
    case TAKEOFF:
      ...
      if (obj_dest != NULL)
      {
        ...
        snprintf(stmess, MAXTEXTLEN, "(%c %d) %s.", Type(obj), obj->pid, GetLocale(L_TAKINGOFF));
        ShellTitle(1, stmess, NULL, NULL, 0, 0);
      }
      break;
          ...
          switch (id1)
          {
            ...
            case 2: /* FIGHTER */
              ...
              if (status == SZ_OK)
              {
                ...
                snprintf(stmess, MAXTEXTLEN, "%s %s.", GetLocale(L_FIGHTER), GetLocale(L_BUYED));
                ShellTitle(1, stmess, NULL, NULL, 0, 0);
              }
              break;
            case 3: /* TOWER */
              ...
              if (status == SZ_OK)
              {
                ...
                snprintf(stmess, MAXTEXTLEN, "%s %s.", GetLocale(L_TOWER), GetLocale(L_BUYED));
                ShellTitle(1, stmess, NULL, NULL, 0, 0);
              }
              break;
            case 4: /* FREIGHTER */
            ...
            case 5: /* SATELLITE */
              ...
              {
                printf("%s %s.\n", GetLocale(L_SATELLITE), GetLocale(L_BUYED));
                snprintf(stmess, MAXTEXTLEN, "%s %s.", GetLocale(L_SATELLITE), GetLocale(L_BUYED));
                ShellTitle(1, stmess, NULL, NULL, 0, 0);
              }
              break;
            case 6: /* CLOAK DEVICE */
              ...
              if (status == SZ_OK)
              {
                ...
                snprintf(stmess, MAXTEXTLEN, "%s %s.", GetLocale(L_CLOAKDEVICE), GetLocale(L_BUYED));
                ShellTitle(1, stmess, NULL, NULL, 0, 0);
              }
    ...
    case SELL:
      if (obj->mode == LANDED)
      {
        printf("%s: %d\n",
               ...
               obj->pid);
        snprintf(stmess, MAXTEXTLEN, "%s: %d", GetLocale(L_SELLINGSPACESHIP), obj->pid);
        ShellTitle(1, stmess, NULL, NULL, 0, 0);
    ...
    case UPGRADE:
        ...
        if (price > 0)
        {
          if (ps->gold > price)
          {
            ...
            printf("(%c %d) %s %d.\n",
                   ...
                   obj->level);
            snprintf(stmess, MAXTEXTLEN, "(%c %d) %s %d.", Type(obj), obj->pid, GetLocale(L_UPGRADETOLEVEL), obj->level);
            ShellTitle(1, stmess, NULL, NULL, 0, 0);
          }
          else
          {
      ...
      else
      {
        printf("%s %d\n",
               ...
               ps->gmaxlevel - 1);
        snprintf(stmess, MAXTEXTLEN, "%s %d", GetLocale(L_YOUCANUPGRADE), ps->gmaxlevel - 1);
        ShellTitle(1, stmess, NULL, NULL, 0, 0);
        ret = NULL;
      }
      break;
...
void
SelectionBox(GdkGC* color, Object** pcv, int reset)
{
  /* version 03*/
    ...
    if (keys.mleft == FALSE)
    {
      ...
      { /* mouse release */
        ...
        if (region.rect.width == 0 || region.rect.height == 0)
        {
          ...
          Window2Real(cv, view, region.rect.x + region.rect.width, region.rect.y + region.rect.height, &x1, &y1);
...
int
Keystrokes(int action, guint* kval, char* c)
{
  /*
...
int
Get2Args(char* cad, char* arg1, char* arg2)
{
  /*
...
void
ShellTitle(int order, char* mess, GdkGC* color, GdkFont* font, int x, int y)
{
