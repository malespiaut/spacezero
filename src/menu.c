...
struct Validargum validarg[] = {{"h", ARG_h}, {"g", ARG_g}, {"n", ARG_n}, {"p", ARG_p}, {"t", ARG_t}, {"l", ARG_l}, {"s", ARG_s}, {"c", ARG_c}, {"ip", ARG_ip}, {"port", ARG_port}, {"name", ARG_name}, {"nosound", ARG_sound}, {"nomusic", ARG_music}, {"soundvol", ARG_soundvol}, {"musicvol", ARG_musicvol}, {"k", ARG_k}, {"font", ARG_font}, {"geom", ARG_geom}, {"cooperative", ARG_cooperative}, {"compcooperative", ARG_compcooperative}, {"queen", ARG_queen}, {"pirates", ARG_pirates}, {"nopirates", ARG_nopirates}, {"enemyknown", ARG_enemyknown}, {"noenemyknown", ARG_noenemyknown}, {"nomenu", ARG_nomenu}, {"fontlist", ARG_fontlist}, {"lang", ARG_lang}, {"", ARG_0}};

struct MenuHead*
MenuHeadNew(char* title)
{
  struct MenuHead* mh;
  int ret;
...
int
Add2MenuHead(struct MenuHead* mhead, struct MenuItem* item0, char* title)
{
  /*
...
void
PrintMenuHead(struct MenuHead* mh)
{
...
void
PrintAllMenu(struct MenuHead* mh)
{
...
char*
GetTextEntry(struct MenuItem* item, char* text)
{
...
int
UpdateMenu(struct MenuHead* mhead, struct MenuHead* mactual)
{
  int ret = 0;
...
void
MenuDown(struct MenuHead* mhead)
{
  struct MenuItem* item;
  int sw = 0;
...
int
MenuEnter(struct MenuHead* mhead)
{
  struct MenuItem* item;
  int sw = 0;
...
void
MenuItemActive(struct MenuHead* mhead, int value)
{
  /*
...
void
Funct01(struct MenuItem* item, char* value)
{
  /*
    Applied the changes.
   */
...
int
Arguments(int argc, char* argv[], char* optfile)
{
  /*
    version 01
    funcion Arguments().
...
#if DEBUG
    if (debug)
      fprintf(stderr, "\tmusic OFF:ERROR\n");
#endif
  }
...
#if DEBUG
    if (debug)
      fprintf(stderr, "\tsound OFF:ERROR\n");
#endif
  }
...
#if DEBUG
    if (debug)
      fprintf(stderr, "\tuniverse size:ERROR\n");
#endif
  }
...
#if DEBUG
    if (debug)
      fprintf(stderr, "\tcooperative:ERROR\n");
#endif
  }
...
#if DEBUG
    if (debug)
      fprintf(stderr, "\tcomputer cooperative:ERROR\n");
#endif
  }
...
#if DEBUG
    if (debug)
      fprintf(stderr, "\tQueen mode:ERROR\n");
#endif
    fsw = 11;
  }
...
void
SetDefaultKeyValues(int action)
{
  int i;
