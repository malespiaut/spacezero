...
int
SelectSector(struct HeadIntIList* hlist, Object* obj, int* a, int* b)
{
  /*
  ...
  *b = y;
  ...
  if (n == 0)
  { /* if no sector found, a near random one inside Universe */
    i = j = 0;
    if (IsInUniverse(x, y))
    {
      i = (int)(6 * (Random(-1)) - 3);
...
int
NearestSector(struct HeadIntIList* hlist, float a, float b, int n)
{
  /*
    version 03
...
int
NearRandomSector(struct HeadIntIList* hlist, float a, float b, int n)
{
  /*
  ...
  */
  ...
  while (cont < 1 && inside == 0)
  { /* avoiding stay on universe border */
