...
void
DrawPoint(GdkGC* gc, int x, int y)
{
  gdk_draw_point(pixmap, gc, x, y);
}
void
DrawLine(GdkGC* gc, int x1, int y1, int x2, int y2)
{
  gdk_draw_line(pixmap, gc, x1, y1, x2, y2);
}
void
DrawRectangle(GdkGC* gc, int fill, int x, int y, int w, int h)
{
  gdk_draw_rectangle(pixmap, gc, fill, x, y, w, h);
}
...
void DrawString_1(GdkFont* font, GdkGC* gc, gint x, gint y, const gchar* string);

GtkWidget*
InitGraphics(char* title, char* optfile, int w, int h, struct Parametres param)
{
  GtkWidget* vbox;
  GtkWidget* menubar;
  ...
  gtk_widget_set_events(win_main, GDK_EXPOSURE_MASK | GDK_LEAVE_NOTIFY_MASK | GDK_KEY_PRESS_MASK | GDK_KEY_RELEASE_MASK);
  /*			|GDK_POINTER_MOTION_MASK */
  /*			|GDK_BUTTON_PRESS_MASK */
  /*			|GDK_BUTTON_RELEASE_MASK */
  /*  			|GDK_POINTER_MOTION_HINT_MASK); */

  gtk_widget_set_events(d_a, GDK_EXPOSURE_MASK | GDK_LEAVE_NOTIFY_MASK | GDK_KEY_PRESS_MASK | GDK_KEY_RELEASE_MASK | GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_POINTER_MOTION_HINT_MASK);

  menuitemsave = gtk_menu_item_new_with_label(GetLocale(L_MSAVE));
  gtk_signal_connect(GTK_OBJECT(menuitemsave), "activate", GTK_SIGNAL_FUNC(Save), NULL);
  gtk_menu_bar_append(GTK_MENU_BAR(menubar), menuitemsave);

  menuitemload = gtk_menu_item_new_with_label(GetLocale(L_MLOAD));
  gtk_signal_connect(GTK_OBJECT(menuitemload), "activate", GTK_SIGNAL_FUNC(Load), NULL);
  ...
  gtk_signal_connect(GTK_OBJECT(menuitemoptions), "activate", GTK_SIGNAL_FUNC(ShowWindowOptions), optfile);

  gtk_signal_connect(GTK_OBJECT(winoptions), "destroy", GTK_SIGNAL_FUNC(QuitWindow), winoptions);
  gtk_signal_connect(GTK_OBJECT(winoptions), "delete_event", GTK_SIGNAL_FUNC(QuitWindow), winoptions);
  ...
  gtk_signal_connect(GTK_OBJECT(menuitemabout), "activate", GTK_SIGNAL_FUNC(ShowWindow), winabout);

  gtk_signal_connect(GTK_OBJECT(winabout), "destroy", GTK_SIGNAL_FUNC(QuitWindow), winabout);
  gtk_signal_connect(GTK_OBJECT(winabout), "delete_event", GTK_SIGNAL_FUNC(QuitWindow), winabout);
  ...
  gtk_signal_connect(GTK_OBJECT(about2), "clicked", GTK_SIGNAL_FUNC(QuitWindow), winabout);
  ...
  gtk_signal_connect(GTK_OBJECT(help2), "clicked", GTK_SIGNAL_FUNC(QuitWindow), winhelp);
  ...
  gtk_signal_connect(GTK_OBJECT(options1), "toggled", GTK_SIGNAL_FUNC(PrintMessage), "option1");

  options2 = gtk_check_button_new_with_label(GetLocale(L_MUSICOFF)); /* "Music OFF"); */
  gtk_widget_show(options2);
  gtk_signal_connect(GTK_OBJECT(options2), "toggled", GTK_SIGNAL_FUNC(PrintMessage), "option2");

  options3 = gtk_check_button_new_with_label(GetLocale(L_SOUNDOFF)); /* "Sound OFF" */
  gtk_widget_show(options3);
  gtk_signal_connect(GTK_OBJECT(options3), "toggled", GTK_SIGNAL_FUNC(PrintMessage), "option3");

  /**** num of planets ****/
  snprintf(label, 128, " %s: (%d,%d)", GetLocale(L_NOFPLANETS), /* "num. of planets" */
           MINNUMPLANETS,
           MAXNUMPLANETS);
  ...
  /**** num of players ****/
  snprintf(label, 128, " %s: (%d,%d)", GetLocale(L_NOFPLAYERS), /* "num. of players" */
           MINNUMPLAYERS,
           MAXNUMPLAYERS);
  ...
  /**** Universe size ****/
  snprintf(label, 128, " %s: (%d,%d)", GetLocale(L_UNIVERSESIZE), /* "Universe Size" */
           MINULX,
           MAXULX);
  ...
  /**** Number of Galaxies ****/
  snprintf(label, 128, " %s: (%d,%d)", GetLocale(L_NOFGALAXIES), /*"Number of Galaxies"*/
           MINNUMGALAXIES,
           MAXNUMGALAXIES);
  ...
  gtk_signal_connect(GTK_OBJECT(options10), "clicked", GTK_SIGNAL_FUNC(SetDefaultOptions), "set default");

  options11 = gtk_button_new_with_label(GetLocale(L_SAVE)); /* " Save " */
  gtk_widget_show(options11);
  gtk_signal_connect(GTK_OBJECT(options11), "clicked", GTK_SIGNAL_FUNC(SaveWindowOptions), optfile);

  options12 = gtk_button_new_with_label(GetLocale(L_CANCEL)); /*" Cancel " */
  gtk_widget_show(options12);
  gtk_signal_connect(GTK_OBJECT(options12), "clicked", GTK_SIGNAL_FUNC(QuitWindow), winoptions);

  /* snprintf(label,128," Changes will take effect after rerun spacezero.\n (sound and music when save)"); */
  snprintf(label, 128, "%s.\n%s", GetLocale(L_CHANGESWILL), GetLocale(L_SOUNDANDMUSICWHENSAVE));
  ...
  gtk_signal_connect(GTK_OBJECT(options14), "toggled", GTK_SIGNAL_FUNC(PrintMessage), "option14");

  options15 = gtk_check_button_new_with_label(GetLocale(L_COMPUTER)); /* "Computer cooperative mode" */
  gtk_widget_show(options15);
  gtk_signal_connect(GTK_OBJECT(options15), "toggled", GTK_SIGNAL_FUNC(PrintMessage), "option15");

  options16 = gtk_check_button_new_with_label(GetLocale(L_QUEEN)); /* "Queen mode" */
  gtk_widget_show(options16);
  gtk_signal_connect(GTK_OBJECT(options16), "toggled", GTK_SIGNAL_FUNC(PrintMessage), "option16");
  ...
  gtk_signal_connect(GTK_OBJECT(win_main), "destroy", GTK_SIGNAL_FUNC(QuitGraphics), NULL);

  gtk_signal_connect(GTK_OBJECT(d_a), "expose_event", GTK_SIGNAL_FUNC(expose_event), NULL);
  /*    gtk_signal_connect(GTK_OBJECT(d_a),"size_request",    */
  /*      		     GTK_SIGNAL_FUNC(SizeRequest),NULL);    */
  gtk_signal_connect(GTK_OBJECT(d_a), "configure_event", GTK_SIGNAL_FUNC(configure_event), NULL);

  gtk_signal_connect(GTK_OBJECT(win_main), "key_press_event", GTK_SIGNAL_FUNC(key_press), NULL);

  gtk_signal_connect(GTK_OBJECT(win_main), "key_release_event", GTK_SIGNAL_FUNC(key_release), NULL);

  gtk_signal_connect(GTK_OBJECT(win_main), "focus_in_event", GTK_SIGNAL_FUNC(GotFocus), NULL);
  gtk_signal_connect(GTK_OBJECT(win_main), "focus_out_event", GTK_SIGNAL_FUNC(LostFocus), NULL);

  /* mouse */
  gtk_signal_connect(GTK_OBJECT(d_a), "motion_notify_event", GTK_SIGNAL_FUNC(motion_notify), NULL);
  gtk_signal_connect(GTK_OBJECT(d_a), "button_press_event", GTK_SIGNAL_FUNC(button_press), NULL);
  gtk_signal_connect(GTK_OBJECT(d_a), "button_release_event", GTK_SIGNAL_FUNC(button_release), NULL);
...
InitColors(void)
{
  ...
  penCloak = GetPen(NewColor(0, 29300, 29300), pixmap);
}
...
gint
QuitGraphics(GtkWidget* widget, gpointer gdata)
{
...
gint
SizeRequest(GtkWidget* widget, GdkEventConfigure* event)
{
  /* The window has been resized */
...
gint
GotFocus(GtkWidget* widget, gpointer data)
{
  keys.ctrl = FALSE;
  return (0);
}
...
gint
LostFocus(GtkWidget* widget, gpointer data)
{
...
void
MousePos(int order, int* x, int* y)
{
...
GdkColor*
NewColor(int red, int green, int blue)
{
...
GdkGC*
GetPen(GdkColor* c, GdkPixmap* pixmap)
{
...
int
DrawObjs(struct HeadObjList* lhc, struct Habitat habitat, Object* cv, Vector r_rel)
{
  /*
    version 0.1
  ...
  */
  ...
  while (ls != NULL)
  {
    ...
    switch (ls->obj->type)
    {
      case PROJECTILE:
        switch (ls->obj->subtype)
        {
          ...
          case MISSILE: /*SHOT3: */
            ...
            gc = gcolors[ps[ls->obj->player].color];
            ...
            for (i = 0; i < missile[0] - 1; i++)
            {
              ...
              gdk_draw_line(pixmap, gc, x + x_0, gheight - (y + y_0), x + x_1, gheight - (y + y_1));
            }
          ...
          case LASER: /*SHOT4: */
            ...
            gdk_draw_line(pixmap, penBlue, x0, gheight - (y0), x1, gheight - (y1));
          ...
          case LASERBEAM:
            ...
            gdk_draw_line(pixmap, penRed, x0, gheight - (y0), x1, gheight - (y1));
            /*	printf("%d %f %f %d\n",x,x0,x1,r); */
            break;
          case EXPLOSION:
            gcexp = gcolors[ps[ls->obj->player].color];
        ...
        switch (ls->obj->subtype)
        {
          ...
          case PILOT:
            ...
            else
            {
              ...
              if (ls->obj->cloak > 0)
                gc = penCloak;
              if (ls->obj->selected == TRUE && ls->obj != cv && ls->obj->player == cv->player)
                gc = penGrey;
      ...
      case ASTEROID:
        DrawAsteroid(x, y, ls->obj);
        break;

      case PLANET:
        DrawPlanet(x, y, ls->obj->radio);
        break;
...
void
DrawShip(GdkGC* gc, int x, int y, Object* obj)
{
  /*
  ...
  switch (obj->subtype)
  {
    ...
    case GOODS:
      ...
      if (obj->accel > 0)
      {
        ...
        gdk_draw_line(pixmap, penCyan, x + x_0, gheight - (y + y_0), x + x_2, gheight - (y + y_2));
        gdk_draw_line(pixmap, penCyan, x + x_1, gheight - (y + y_1), x + x_2, gheight - (y + y_2));
      }
      /********** --engine flares *************/
    ...
    case SATELLITE:
      gdk_draw_arc(pixmap, gc, FALSE, x - obj->radio, gheight - y - obj->radio, 2 * obj->radio, 2 * obj->radio, 0, 23040);
      break;

    case TOWER:
      if (obj->player != ppirates)
      {
        gdk_draw_rectangle(pixmap, gc, FALSE, x - .5 * obj->radio, gheight - y - obj->radio, obj->radio, 2 * obj->radio);
      }
      else
      {
        x0 = x - 0.5 * obj->radio;
...
DrawPlanet(int x, int y, int r)
{
  ...
  */
  ...
  gdk_draw_arc(pixmap, penBlue, TRUE, x - r, gheight - y - r, 2 * r, 2 * r, 0, 23040);
...
void
DrawStars(int mode, float rx, float ry)
{
  /*
    fixed stars
  */
  ...
  if (mode == RELATIVE)
  {
    ...
    for (i = 0; i < numstars; i++)
    {
      x = stars.x[i] - rrelxw;
      y = stars.y[i] - rrelyh;
      gdk_draw_point(pixmap, pen[stars.color[i]], x + gwidth * ((x < 0) - (x > gwidth)), gheight * (1 - ((y < 0) - (y > gheight))) - y);
    }
  }
  else
  {
  ...
  */
  ...
  while (s != NULL)
  {
    ...
    switch (s->type)
    {
      case TERRAIN:
        gc = penWhite;
        gdk_draw_line(pixmap, gc, sx * s->x0, gheight - sy * s->y0, sx * s->x1, gheight - sy * s->y1);
        break;
      case LANDZONE:
        gc = gcolors[ps[obj->player].color];
        gdk_draw_line(pixmap, gc, sx * s->x0, gheight - sy * s->y0 + 1, sx * s->x1, gheight - sy * s->y1 + 1);
        gdk_draw_line(pixmap, gc, sx * s->x0, gheight - sy * s->y0 + 2, sx * s->x1, gheight - sy * s->y1 + 2);
        break;
      default:
        break;
    }
...
int
DrawRadar(Object* obj, struct HeadObjList* lhc, int crash)
{
  /*
  ...
  */
  ...
  while (ls != NULL)
  {
    ...
    if ((rx * rx + ry * ry) * (1 + ls->obj->cloak) > (float)obj->radar * (float)obj->radar)
    {
      ls = ls->next;
      continue;
    }
    ...
    switch (ls->obj->type)
    {
      case SHIP:
        ...
        {
          if (ps[ls->obj->player].team == ps[obj->player].team)
          {
            gc = penBlue;
          }
          else
...
void
DrawSpaceShip(Object* obj, struct HeadObjList* lhc)
{
  /*
  ...
  for (i = 0; i < s[0]; i++)
  {
    ...
    switch ((int)s[j])
    {
      ...
      case LINE:
        ...
        y2 = s[++j];
        j++;
        gdk_draw_line(pixmap, gc, x0 + x1 * fx, gheight - (y0 + y1 * fy), x0 + x2 * fx, gheight - (y0 + y2 * fy));
        break;
      case CIRCLE:
        break;
      default:
...
void
DrawAsteroid(int x, int y, Object* obj)
{
  /*
    version 01 25Oct10
    Draw an asteroid
...
void
DrawMap(int player, struct HeadObjList hol, Object* cv, int ulx)
{
  /*
    version 02
  ...
  */
  ...
  if (keys.order.state == FALSE)
  {
    ...
    /* 		    "O: Introduce command");  */
    ShellTitle(0, NULL, penRed, gfont, 10, gheight + GameParametres(GET, GPANEL, 0) / 2 + 4);
  ...
  while (ls != NULL)
  {
    switch (ls->obj->type)
    {
      case SHIP:
        ...
        if (!genemyknown)
        {
          if (ps[ls->obj->player].team != ps[player].team)
          {
            ls = ls->next;
            continue;
          }
        ...
        break;
      case PLANET:
        if (IsInIntList((ps[player].kplanets), ls->obj->id) == 0)
        {
          ls = ls->next;
          continue;
        }
    ...
    switch (ls->obj->type)
    {
      case PLANET:
        ...
        if (label & 1)
        {
          ...
          DrawString(gfont, gc, x + 5, gheight - y, point);
        }
      ...
      case ASTEROID:
        ...
        if (ls->obj->type == ASTEROID)
        { /* don't draw far asteroids */
  ...
  DrawString(gfont, penGreen, x0 + 10, y0 + 5, point);
  ...
  DrawString(gfont, penGreen, x, y, point);
  ...
  DrawString(gfont, penGreen, x, y - texth - 2, point);
...
float
Distance(Object* obj, float x, float y)
{
  /*
...
void
DrawInfo(Object* obj, struct Draw* gdraw, struct HeadObjList* lheadobjs, struct TextMessageList* lheadtext)
{
  /*
  ...
  */
  ...
  x += 15 * charw;
  gc = penRed;
  sprintf(point, "T:%d   Ss:%d   P:%d   Ast:%d  Tr:%d  Pi:%d Ob/s(%.1f) FPS:%.1f", CountObjs(lheadobjs, -1, -1, -1), CountObjs(lheadobjs, -1, SHIP, -1), CountObjs(lheadobjs, -1, PROJECTILE, -1), CountObjs(lheadobjs, -1, ASTEROID, -1), CountObjs(lheadobjs, -1, TRACE, -1), CountObjs(lheadobjs, -1, SHIP, PILOT), (float)(20.0 * nobjsend / 100), fps);
  DrawString(gfont, gc, x, y, point);

  sprintf(point, "T:%d   E:%d   F:%d   T:%d Pi:%d", CountObjs(lheadobjs, obj->player, -1, -1), CountObjs(lheadobjs, obj->player, SHIP, EXPLORER), CountObjs(lheadobjs, obj->player, SHIP, FIGHTER), CountObjs(lheadobjs, obj->player, SHIP, TOWER), CountObjs(lheadobjs, obj->player, SHIP, PILOT));

  DrawString(gfont, gc, x, y + 15, point);
#else
  /* fps */
  x = wwidth - 10 * charw;
  sprintf(point, "FPS:%.1f", fps);
  DrawString(gfont, penRed, x, y, point);
  /* --fps */
#endif
  ...
  if (keys.order.state == FALSE)
  {
    ...
    /* 	       "O: Introduce command"); */
    ShellTitle(0, NULL, penRed, gfont, 10, wheight + GameParametres(GET, GPANEL, 0) / 2 + 4);
  }
  /*
   *    Ship info
   */
  if (obj != NULL)
  {
    ...
    {
      y = DrawShipInfo(gfont, penGreen, obj, 0, 0);

      sprintf(point, "============");
      DrawString(gfont, penGreen, 10, y, point);
      y += incy;
    }
  }
  ...
  if (gdraw->info == TRUE)
  {
    y = DrawPlayerInfo(gfont, penCyan, &ps[obj->player], 10, y);
    sprintf(point, "============");
    DrawString(gfont, penGreen, 10, y, point);
    y += incy;
    ...
    if (obj != NULL)
    {
      if (obj->habitat == H_PLANET)
      {
        ...
        if (planet != NULL)
        {
          ...
          y = DrawPlanetInfo(gfont, penGreen, planet, obj->player, 10, y);
          sprintf(point, "============");
          DrawString(gfont, penGreen, 10, y, point);
          y += incy;
        }
  ...
  {
    if (d2 * (1 + objt->cloak) < (obj->radar * obj->radar))
    {
      DrawEnemyShipInfo(gfont, penGreen, objt, wwidth, 0);
    }
  }
  ...
  if (PendingTextMessage())
  {
    ...
    DrawMessageBox(gfont, point, wwidth2, (int)(.25 * wheight), MBOXBORDER);
  }
  ...
  while (lh != NULL)
  {
    ...
    if (lh->info.duration > 0 && i < 10)
    {
      ...
      */
      ...
      DrawString(gfont, gcmessage, 20, wheight - incy * swgmess + incy * i + 5, point);
      if (gfont != NULL)
      {
...
int
DrawPlayerInfo(GdkFont* font, GdkGC* color, struct Player* player, int x0, int y0)
{
  /*
  ...
  */
  ...
  DrawString(font, color, x0, y, point);
  y += incy;
...
int
DrawPlanetInfo(GdkFont* font, GdkGC* color, Object* planet, int player, int x0, int y0)
{
  /*
...
int
DrawEnemyShipInfo(GdkFont* font, GdkGC* color, Object* obj, int x0, int y0)
{
  /*
  ...
  */
  ...
  if (font != NULL)
  {
    ...
    DrawString(font, color, x, y + texth, GetLocale(L_ENERGY));
    y += incy;
    DrawString(font, color, x, y + texth, GetLocale(L_STATE));
    y += incy;
    /* production */
    sprintf(cad, "L : %d", obj->level);
    DrawString(font, color, x, y + texth, cad);
    y += incy;
  }
  ...
  DrawBarBox(color, gc, x + textw + 10, y, sx, texth - 1, n);
  y += incy;
...
int
DrawShipInfo(GdkFont* font, GdkGC* color, Object* obj, int x0, int y0)
{
  /*
  ...
  */
  ...
  DrawBarBox(color, gc, x, y, sx, texth - 1, n);
  snprintf(point, MAXTEXTLEN, "%s %.0f", GetLocale(L_ENERGY), obj->gas);
  DrawString(font, color, x + sx + 10, y + texth, point);
  y += incy;
  ...
  if (obj->cargo.capacity > 0)
  {
    ...
    DrawBarBox(color, gc, x, y, sx, texth - 1, n);
    snprintf(point, MAXTEXTLEN, "%s (%d/%d)", "CARGO", obj->cargo.mass, obj->cargo.capacity);
    DrawString(font, color, x + sx + 10, y + texth, point);
    y += incy;
  }
  ...
  DrawBarBox(color, gc, x, y, sx, texth - 1, a);
  ...
  if (obj->cargo.hlist != NULL)
  {
    ...
    if (np > 0)
    {
      ...
      DrawShip(color, x0 + x + cargoobj.radio, -(y + texth) + gheight, &cargoobj);
      gdk_draw_rectangle(pixmap, color, FALSE, x0 + x, y + texth - texth, 2 * texth, 2 * texth);
      snprintf(point, MAXTEXTLEN, "%d", np);
      DrawString(font, color, x0 + x + 2 * texth + 5, y + texth - texth + 2 * texth, point);
      x += 4 * texth;
    }
    if (ns > 0)
    {
      ...
      cargoobj.radio = 5;
      DrawShip(color, x0 + x + 2 * cargoobj.radio, -(y + texth) + gheight, &cargoobj);
      gdk_draw_rectangle(pixmap, color, FALSE, x0 + x, y + texth - texth, 2 * texth, 2 * texth);
  ...
  if (obj->weapon0.cont1)
    wgc = penRed;
  DrawString(font, wgc, x0 + x, y + texth, "1");
  ...
  if (obj->weapon1.cont1)
    wgc = penRed;
  DrawString(font, wgc, x0 + x, y + texth, "2");
  ...
  if (obj->weapon2.cont1)
    wgc = penRed;
  DrawString(font, wgc, x0 + x, y + texth, "3");
  ...
  snprintf(point, MAXTEXTLEN, "%s %d   %d", GetLocale(L_SECTOR), (int)(ox / SECTORSIZE) - (ox < 0), (int)(oy / SECTORSIZE) - (oy < 0));
  DrawString(font, color, x0 + x, y + texth, point);
  y += incy;
  ...
  if (ord != NULL)
  {
    ...
    switch (ord->id)
    {
      case GOTO:
        ...
        {
          snprintf(point, MAXTEXTLEN, "%s %s: (%d,%d)", GetLocale(L_ORDER), GetLocale(L_GOTO), (int)(ord->a / SECTORSIZE) - (ord->a < 0), (int)(ord->b / SECTORSIZE) - (ord->b < 0));
          dx = ox - ord->a;
          dy = oy - ord->b;
          d = sqrt(dx * dx + dy * dy) / SECTORSIZE;
        }
      ...
      case RETREAT:
        if (ord->c != -1)
        {
          snprintf(point, MAXTEXTLEN, "%s: %s %d", GetLocale(L_ORDER), GetLocale(L_RETREAT), (int)ord->e);
          dx = ox - ord->a;
          dy = oy - ord->b;
          d = sqrt(dx * dx + dy * dy) / SECTORSIZE;
        }
        ...
        {
          snprintf(point, MAXTEXTLEN, "%s: %s (%d,%d)", GetLocale(L_ORDER), GetLocale(L_RETREAT), (int)(ord->a / SECTORSIZE) - (ord->a < 0), (int)(ord->b / SECTORSIZE) - (ord->b < 0));
          dx = ox - ord->a;
          dy = oy - ord->b;
          d = sqrt(dx * dx + dy * dy) / SECTORSIZE;
        }
  ...
  /* --AUTO FREIGHTER */
  snprintf(point, MAXTEXTLEN, "%s: %.1f %s", GetLocale(L_DISTANCE), d, GetLocale(L_SECTORS));
  DrawString(font, color, x0 + x, y + texth, point);
  y += incy;
...
int
DrawTextList(GdkFont* font, char* title, struct HeadTextList* head, int x0, int y0, int width, int height)
{
  /*
  ...
  */
  ...
  DrawString(font, penBlue, x, y, title);
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (y - scroll * incy > y0)
    {
      DrawString(font, gc, x, y + incy - scroll * incy, lh->text);
  ...
  gdk_draw_rectangle(pixmap, penGreen, FALSE, x0, y0, textw + charw + 10, height);

  return (0);
}

int
DrawMenuHead(GdkFont* font, struct MenuHead* head, int x0, int y0)
{
  /*
     print the text list head in pixmap at the position x0, y0.
  */
  ...
  DrawString(font, gc, x, y - scroll * incy, head->title);
  y += 2 * incy;
  ...
  while (item != NULL)
  {
    ...
    DrawString(font, gc, x, y - scroll * incy, point);
    if (item->active == ITEM_ST_EDIT)
    {
      if (strlen(point2) > 0)
      {
        ...
        DrawString(font, gc, x + w, y - scroll * incy, point2);
      }
    }
    item = item->next;
    y += incy;
...
gint
Load(GtkWidget* widget, gpointer gdata)
{
...
gint
Save(GtkWidget* widget, gpointer gdata)
{
  keys.save = TRUE;
  if (GameParametres(GET, GMODE, 0) == CLIENT)
    keys.save = FALSE;
...
gint
Options(GtkWidget* widget, gpointer gdata)
{
  return (0);
}
...
gint
SetDefaultOptions(GtkWidget* widget, gpointer gdata)
{
...
gint
PrintMessage(GtkWidget* widget, gpointer gdata)
{
  /* production */
  return (0);
  g_print("%s\n", (char*)gdata);
...
gint
ShowWindowOptions(GtkWidget* widget, gpointer gdata)
{
  /*
    version 01 12May11
  */
...
gint
SaveWindowOptions(GtkWidget* widget, gpointer gdata)
{
...
gint
ShowWindow(GtkWidget* widget, gpointer gdata)
{
...
gint
QuitWindow(GtkWidget* widget, gpointer gdata)
{
...
void
DrawPlayerList(int player, struct HeadObjList* hlp, Object* cvobj, int act)
{
  /*
  ...
  */
  ...
  snprintf(titleships, MAXTEXTLEN, "SHIPS: E:%d F:%d T:%d Q:%d C:%d", ships[EXPLORER], ships[FIGHTER], ships[TOWER], ships[QUEEN], ships[FREIGHTER]);
    ...
    while (ls != NULL)
    {
      ...
      switch (obj->type)
      {
          ...
          if (ord != NULL)
          {
            ...
            switch (ord->id)
            {
              ...
              case GOTO:
                ...
                {
                  snprintf(cad, MAXTEXTLEN, "%s(%d,%d) ", tmpcad, (int)(ord->a / SECTORSIZE) - (ord->a < 0), (int)(ord->b / SECTORSIZE) - (ord->b < 0));
                }
                break;
              case EXPLORE:
        ...
        case PLANET:
          ...
          snprintf(cad, MAXTEXTLEN, "id:%d g:%.0f  %d", obj->id, obj->planet->gold, shipsinplanets[obj->id - 1]);
          textw = gdk_text_width(gfont, cad, strlen(cad));
          if (textw > textpw)
            textpw = textw;
          Add2TextList(&hplanetlist, cad, color);
  ...
  DrawTextList(gfont, titleships, &hshiplist, 10, 15, textsw + charw + 10, GameParametres(GET, GHEIGHT, 0) - 50);

  DrawTextList(gfont, "PLANETS:", &hplanetlist, GameParametres(GET, GWIDTH, 0) - textpw - 20, 15, textpw + charw + 10, GameParametres(GET, GHEIGHT, 0) - 50);

  return;
}

void
Shift(int action, int ulx, int objid, float* z, float* x, float* y)
{
...
void
Window2Real(Object* cv, int view, int wx, int wy, int* rx, int* ry)
{
  /* version 01 */
  float zoom;
  float cvx, cvy;
...
void
Real2Window(Object* cv, int view, int rx, int ry, int* wx, int* wy)
{
  /* version 01 */
...
void
Window2Sector(Object* cv, int* x, int* y)
{
  /*
...
void
Real2Sector(int x, int y, int* a, int* b)
{
  /*
...
void
DrawSelectionBox(GdkGC* color, int view, Space reg, Object* cv)
{
  Rectangle rect;
...
int
DrawGameStatistics(struct Player* pl)
{
  /*
  ...
  */
  ...
  DrawString(gfont, penWhite, x, y + textheight, cad);
  ...
  /* HERE: send kills and deaths to client */
  {
    ...
    for (i = 0; i < nplayers - 1; i++)
    {
      ...
      { /* HERE set default name */
        snprintf(cad, MAXTEXTLEN, "%d: %s, [L%d-%d : %d : %1.1f], %s: %d, %s: %d (%d), %s: %d, %s: %d. %s %d", i + 1, pl[a[i]].playername, pl[a[i]].gmaxlevel, pl[a[i]].maxlevel, pl[a[i]].level + pl[a[i]].nships, (float)(pl[a[i]].level) / (pl[a[i]].nships + 0.0001), GetLocale(L_PLANETS), pl[a[i]].nplanets, GetLocale(L_SHIPS), pl[a[i]].nships, pl[a[i]].nbuildships, GetLocale(L_KILLS), pl[a[i]].nkills, GetLocale(L_DEATHS), pl[a[i]].ndeaths, "spend:", pl[a[i]].goldships + pl[a[i]].goldupdates);
      }
      else if (pl[a[i]].status == PLAYERDEAD)
      {
        snprintf(cad, MAXTEXTLEN, "%d: %s, [L%d] GAMEOVER, %s: %d (%d), %s: %d, %s: %d, %s: %d. %s %d", i + 1,
                 /* GetLocale(L_dPLAYER), */
                 pl[a[i]].playername,
                 pl[a[i]].gmaxlevel,
                 GetLocale(L_SHIPS),
                 ...
                 pl[a[i]].ndeaths,
                 "spend:",
                 pl[a[i]].goldships + pl[a[i]].goldupdates);
      }
      len = strlen(cad);
      if (len > len0)
      {
      ...
      DrawString(gfont, gcolors[ps[a[i]].color], x, y + (i + 2) * textheight, cad);
    }
    free(a);
  }
  return (x - 10);
...
void
DrawString(GdkFont* font, GdkGC* gc, gint x, gint y, const gchar* string)
{
  /*
...
#if USEPANGO
  DrawString_1(font, gc, x, y, string);
  return;
#else
...
void
DrawText(GdkFont* font, GdkGC* gc, gint x, gint y, const gchar* string)
{
  /*
    not used
...
void
DrawString_1(GdkFont* font, GdkGC* gc, gint x, gint y, const gchar* string)
{
...
void
DrawMessageBox(GdkFont* font, char* cad, int x0, int y0, int type)
{
  /*
  ...
  */
  ...
  DrawString(font, penGreen, x + texth, y + 1.5 * texth, cad);
}

void
DrawCharList(GdkFont* font, GdkGC* color, struct CharListHead* hlist, int x0, int y0)
{
  ...
  */
  ...
  while (list != NULL)
  {
    ...
    DrawString(font, color, x0 + 5, y0 + incy * swgmess + 5, point);
...
void
DrawBarBox(GdkGC* border, GdkGC* color, int x, int y, int w, int h, float value)
{
...
void
DrawWindow(GdkFont* font, GdkGC* color0, int x0, int y0, int type, struct Window* w)
{
    ...
    case 1: /* window with scroll */
      ...
      if (swgmess)
      {
        ...
        /* 		    y0+window.height); */
        {
          ...
          for (i = 0; i <= 5; i++)
          {
            gdk_draw_line(pixmap, penRed, xbar, ybar + y0 - 5 + i * 2, xbar + wbar, ybar + y0 - 5 + i * 2);
          }
          /* arrows */
          gdk_draw_line(pixmap, penRed, xbar + 2, y0 + 8, xbar + wbar / 2, y0 + 2);
          gdk_draw_line(pixmap, penRed, xbar + wbar / 2, y0 + 2, xbar + wbar - 2, y0 + 8);

          gdk_draw_line(pixmap, penRed, xbar + 2, y0 + window.height - 8, xbar + wbar / 2, y0 + window.height);
          gdk_draw_line(pixmap, penRed, xbar + wbar / 2, y0 + window.height, xbar + wbar - 2, y0 + window.height - 8);
        }
        w->x = window.x;
        w->y = window.y;
      ...
      while (list != NULL)
      {
        ...
        DrawString(font, color, x0 + 5, y0 + incy * (swgmess + 1) + 5, point);
        textw = gdk_text_width(font, point, strlen(point));
        if (textw > glen)
          glen = textw;
...
void
PrintFontName(char* fname, int n)
{
  char** fnames;
  int nfonts;
  int i;
...
void
DrawStatistics(Rectangle* r, struct Stats* stats, int stats_n, int stats_np)
{
  int i, j;
  int maxnplanets;
  float maxlevel;
  ...
  for (i = 0; i < stats_np; i++)
  {
    ...
    if (ps[i].level + ps[i].nships > maxlevel)
      maxlevel = ps[i].level + ps[i].nships;
    if (ps[i].nplanets > maxnplanets)
      maxnplanets = ps[i].nplanets;
  }
  ...
  if ((USEPIXMAPS && !(cont % 1)) || !USEPIXMAPS)
  {
    ...
    for (i = 1; i < stats_np; i++)
    {
      ...
      x1 = rect.x + (j + 1) * dx;
      y1 = rect.y + (rect.height - 1) - (ps[i].level + ps[i].nships) * dy;
      gdk_draw_line(pm,
                    gcolors[ps[i].color],
                    x0,
                    y0,
  ...
  if (USEPIXMAPS)
  {
    ...
    rect.width = r->width;
    rect.height = r->height;
    gdk_draw_drawable(pixmap, gc_stat, pixmap_stat, 0, 0, rect.x, rect.y, rect.width, rect.height);
  }
